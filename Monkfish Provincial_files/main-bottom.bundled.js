...
//
(function(){if(window.karmaEnabled){window.adService={isDesktop:window.innerWidth>1024,mobileAds:window.innerWidth<768,unitValues:{channel:adConfiguration.settings.site.replace(/ar.mobile.?/,""),parent:adConfiguration.settings.zone||""},pageTargetingValues:{},};
var a=adConfiguration.pagetargets,f=adService.pageTargetingValues,j;for(j in a){a[j].key=a[j].key.replace(/^.id$/,"id");f[a[j].key]=a[j].value
}window.adServiceQ=window.adServiceQ||[];var g=window.location.search.match(/adTestEnv=([^&]+)/i),i=window.location.search.match(/adTestMin=([^&]+)/i);
window.adService=window.adService||{};adService.filePaths=adService.filePaths||{};adService.useSSL=adService.useSSL||(window.location.protocol=="https:");
adService.environment=(g!==null&&g.length>1)?g[1]:adService.environment||"www";var c,e=(adService.environment!=="www"),d=(adService.environment==="local"),b=adService.environment,h=(i!=null)?(i[1]=="true"):!e;
c=document.createElement("script");if(adService.useSSL){adService.filePaths.adServiceRender="https://"+(e?b+".":"")+"secure.recipe.com/web/"+(h?"js-min/":"")+"js/mdp/app/gpt/gpt.adServiceRender.js"
}else{adService.filePaths.adServiceRender="http://"+(e?b+".":"")+"karma.mdpcdn.com"+(d?":9999":"")+"/service/"+(h?"js-min":"js")+"/karma.service.js"
}c.src=adService.filePaths.adServiceRender;document.body.appendChild(c)}})();
//
// source/js/ads/karma-ads-helper.js
//
if(window.karmaEnabled){(function(){var e=document.getElementById("div-gpt-ad-recipepagemiddle-3")||false;if(adService.isDesktop&&e){window.adServiceQ.push(function(){adService.renderAds.createLazyLoadSlot("div-gpt-lazy-legacyBanner",e,true)
})}window.googletag=window.googletag||{};googletag.cmd=googletag.cmd||[];googletag.cmd.push(function(){googletag.pubads().addEventListener("slotRenderEnded",function(i){h(i)
})});var g=0;var c=false;var d=true;var f=false;var b=0;var a=0;var h=function(j){g++;if(j){var i=$("#"+j.slot.getSlotId().getDomId());
if(j.isEmpty||i.clientHeight===0){var m=i.parents("[data-ad-container-autocollapse]");if(m.length){m.hide()}var k=i.parents("[data-ad-container-masonry-autocollapse]");
var l=i.data("sponsored-slot");if(k.length&&!l){k.hide();c=true}}}if(!c){return}if(d){if(adService.mobileAds){b=1;a=adService.adSlots.length-($("article[data-ad-container-masonry-autocollapse]").length-1)
}else{b=$(".gridad").length;a=adService.adSlots.length}if(a!==g){return}f=true;d=false}else{if(g!==b){return}f=true}if(!f){return
}window.pubsub.broadcast("MasonryGridReload");if(window.console.error){window.console.error("Masonry grid is being reloaded due to an ad slot not being targeted.")
}g=0;c=false;f=false}})()};
//
 ...
 * by David DeSandro
 */
(function(d){var c=Array.prototype.slice;
function b(){}function a(e){if(!e){return}function f(i){if(i.prototype.option){return}i.prototype.option=function(j){if(!e.isPlainObject(j)){return
}this.options=e.extend(true,this.options,j)}}var h=typeof console==="undefined"?b:function(i){console.error(i)};function g(i,j){e.fn[i]=function(p){if(typeof p==="string"){var k=c.call(arguments,1);
for(var m=0,o=this.length;m<o;m++){var l=this[m];var n=e.data(l,i);if(!n){h("cannot call methods on "+i+" prior to initialization; attempted to call '"+p+"'");
continue}if(!e.isFunction(n[p])||p.charAt(0)==="_"){h("no such method '"+p+"' for "+i+" instance");continue}var q=n[p].apply(n,k);
if(q!==undefined){return q}}return this}else{return this.each(function(){var r=e.data(this,i);if(r){r.option(p);r._init()}else{r=new j(this,p);
e.data(this,i,r)}})}}}e.bridget=function(i,j){f(j);g(i,j)};return e.bridget}if(typeof define==="function"&&define.amd){define("jquery-bridget/jquery.bridget",["jquery"],a)
}else{a(d.jQuery)}})(window);
/*
 * eventie v1.0.5
...
var a=function(){};function d(h){var g=f.event;g.target=g.target||g.srcElement||h;return g}if(b.addEventListener){a=function(h,i,g){h.addEventListener(i,g,false)
}}else{if(b.attachEvent){a=function(h,i,g){h[i+g]=g.handleEvent?function(){var j=d(h);g.handleEvent.call(g,j)}:function(){var j=d(h);
g.call(h,j)};h.attachEvent("on"+i,h[i+g])}}}var e=function(){};if(b.removeEventListener){e=function(h,i,g){h.removeEventListener(i,g,false)
}}else{if(b.detachEvent){e=function(i,j,h){i.detachEvent("on"+j,i[j+h]);try{delete i[j+h]}catch(g){i[j+h]=undefined}}}}var c={bind:a,unbind:e};
if(typeof define==="function"&&define.amd){define("eventie/eventie",c)}else{if(typeof exports==="object"){module.exports=c}else{f.eventie=c
}}})(this);
/*
 * docReady
 * Cross browser DOMContentLoaded event emitter
 */
(function(f){var c=f.document;var e=[];function b(g){if(typeof g!=="function"){return
}if(b.isReady){g()}else{e.push(g)}}b.isReady=false;function d(g){var k=g.type==="readystatechange"&&c.readyState!=="complete";
if(b.isReady||k){return}b.isReady=true;for(var j=0,l=e.length;j<l;j++){var h=e[j];h()}}function a(g){g.bind(c,"DOMContentLoaded",d);
g.bind(c,"readystatechange",d);g.bind(f,"load",d);return b}if(typeof define==="function"&&define.amd){b.isReady=typeof requirejs==="function";
 ...
 * @preserve
 */
(function(){function k(){}var t=k.prototype;
var l=this;var s=l.EventEmitter;function p(A,z){var y=A.length;while(y--){if(A[y].listener===z){return y}}return -1}function f(z){return function y(){return this[z].apply(this,arguments)
}}t.getListeners=function n(z){var y=this._getEvents();var B;var A;if(z instanceof RegExp){B={};for(A in y){if(y.hasOwnProperty(A)&&z.test(A)){B[A]=y[A]
}}}else{B=y[z]||(y[z]=[])}return B};t.flattenListeners=function m(A){var y=[];var z;for(z=0;z<A.length;z+=1){y.push(A[z].listener)
}return y};t.getListenersAsObject=function o(y){var z=this.getListeners(y);var A;if(z instanceof Array){A={};A[y]=z}return A||z
};t.addListener=function c(y,A){var C=this.getListenersAsObject(y);var B=typeof A==="object";var z;for(z in C){if(C.hasOwnProperty(z)&&p(C[z],A)===-1){C[z].push(B?A:{listener:A,once:false})
}}return this};t.on=f("addListener");t.addOnceListener=function e(y,z){return this.addListener(y,{listener:z,once:true})};t.once=f("addOnceListener");
t.defineEvent=function g(y){this.getListeners(y);return this};t.defineEvents=function h(y){for(var z=0;z<y.length;z+=1){this.defineEvent(y[z])
}return this};t.removeListener=function v(y,B){var C=this.getListenersAsObject(y);var z;var A;for(A in C){if(C.hasOwnProperty(A)){z=p(C[A],B);
if(z!==-1){C[A].splice(z,1)}}}return this};t.off=f("removeListener");t.addListeners=function d(y,z){return this.manipulateListeners(false,y,z)
};t.removeListeners=function w(y,z){return this.manipulateListeners(true,y,z)};t.manipulateListeners=function q(C,y,A){var z;
var E;var D=C?this.removeListener:this.addListener;var B=C?this.removeListeners:this.addListeners;if(typeof y==="object"&&!(y instanceof RegExp)){for(z in y){if(y.hasOwnProperty(z)&&(E=y[z])){if(typeof E==="function"){D.call(this,z,E)
}else{B.call(this,z,E)}}}}else{z=A.length;while(z--){D.call(this,y,A[z])}}return this};t.removeEvent=function u(z){var B=typeof z;
var y=this._getEvents();var A;if(B==="string"){delete y[z]}else{if(z instanceof RegExp){for(A in y){if(y.hasOwnProperty(A)&&z.test(A)){delete y[A]
}}}else{delete this._events}}return this};t.removeAllListeners=f("removeEvent");t.emitEvent=function j(z,y){var D=this.getListenersAsObject(z);
var C;var A;var B;var E;for(B in D){if(D.hasOwnProperty(B)){A=D[B].length;while(A--){C=D[B][A];if(C.once===true){this.removeListener(z,C.listener)
}E=C.listener.apply(this,y||[]);if(E===this._getOnceReturnValue()){this.removeListener(z,C.listener)}}}}return this};t.trigger=f("emitEvent");
t.emit=function i(z){var y=Array.prototype.slice.call(arguments,1);return this.emitEvent(z,y)};t.setOnceReturnValue=function x(y){this._onceReturnValue=y;
return this};t._getOnceReturnValue=function b(){if(this.hasOwnProperty("_onceReturnValue")){return this._onceReturnValue}else{return true
}};t._getEvents=function a(){return this._events||(this._events={})};k.noConflict=function r(){l.EventEmitter=s;return k};if(typeof define==="function"&&define.amd){define("eventEmitter/EventEmitter",[],function(){return k
})}else{if(typeof module==="object"&&module.exports){module.exports=k}else{this.EventEmitter=k}}}.call(this));
/*
 * getStyleProperty v1.0.3
 * original by kangax
 * http://perfectionkills.com/feature-testing-css-properties/
 */
(function(d){var c="Webkit Moz ms Ms O".split(" ");
var a=document.documentElement.style;function b(h){if(!h){return}if(typeof a[h]==="string"){return h}h=h.charAt(0).toUpperCase()+h.slice(1);
var g;for(var e=0,f=c.length;e<f;e++){g=c[e]+h;if(typeof a[g]==="string"){return g}}}if(typeof define==="function"&&define.amd){define("get-style-property/get-style-property",[],function(){return b
})}else{if(typeof exports==="object"){module.exports=b}else{d.getStyleProperty=b}}})(window);(function(h,g){var b=h.getComputedStyle;
var c=b?function(i){return b(i,null)}:function(i){return i.currentStyle};function d(k){var j=parseFloat(k);var i=k.indexOf("%")===-1&&!isNaN(j);
return i&&j}var f=["paddingLeft","paddingRight","paddingTop","paddingBottom","marginLeft","marginRight","marginTop","marginBottom","borderLeftWidth","borderRightWidth","borderTopWidth","borderBottomWidth"];
function e(){var m={width:0,height:0,innerWidth:0,innerHeight:0,outerWidth:0,outerHeight:0};for(var j=0,k=f.length;j<k;j++){var l=f[j];
m[l]=0}return m}function a(k){var i=k("boxSizing");var l;(function(){if(!i){return}var o=document.createElement("div");o.style.width="200px";
o.style.padding="1px 2px 3px 4px";o.style.borderStyle="solid";o.style.borderWidth="1px 2px 3px 4px";o.style[i]="border-box";var n=document.body||document.documentElement;
n.appendChild(o);var p=c(o);l=d(p.width)===200;n.removeChild(o)})();function j(p){if(typeof p==="string"){p=document.querySelector(p)
}if(!p||typeof p!=="object"||!p.nodeType){return}var B=c(p);if(B.display==="none"){return e()}var A={};A.width=p.offsetWidth;
A.height=p.offsetHeight;var r=A.isBorderBox=!!(i&&B[i]&&B[i]==="border-box");for(var q=0,t=f.length;q<t;q++){var w=f[q];var E=B[w];
E=m(p,E);var x=parseFloat(E);A[w]=!isNaN(x)?x:0}var z=A.paddingLeft+A.paddingRight;var y=A.paddingTop+A.paddingBottom;var v=A.marginLeft+A.marginRight;
var u=A.marginTop+A.marginBottom;var o=A.borderLeftWidth+A.borderRightWidth;var n=A.borderTopWidth+A.borderBottomWidth;var s=r&&l;
var D=d(B.width);if(D!==false){A.width=D+(s?0:z+o)}var C=d(B.height);if(C!==false){A.height=C+(s?0:y+n)}A.innerWidth=A.width-(z+o);
A.innerHeight=A.height-(y+n);A.outerWidth=A.width+v;A.outerHeight=A.height+u;return A}function m(n,s){if(b||s.indexOf("%")===-1){return s
}var r=n.style;var o=r.left;var p=n.runtimeStyle;var q=p&&p.left;if(q){p.left=n.currentStyle.left}r.left=s;s=r.pixelLeft;r.left=o;
if(q){p.left=q}return s}return j}if(typeof define==="function"&&define.amd){define("get-size/get-size",["get-style-property/get-style-property"],a)
}else{if(typeof exports==="object"){module.exports=a(require("get-style-property"))}else{h.getSize=a(h.getStyleProperty)}}})(window);
(function(d,c){var g=(function(){if(c.matchesSelector){return"matchesSelector"}var o=["webkit","moz","ms","o"];for(var k=0,l=o.length;
k<l;k++){var n=o[k];var m=n+"MatchesSelector";if(c[m]){return m}}})();function e(k,l){return k[g](l)}function a(k){if(k.parentNode){return
}var l=document.createDocumentFragment();l.appendChild(k)}function i(k,o){a(k);var l=k.parentNode.querySelectorAll(o);for(var m=0,n=l.length;
m<n;m++){if(l[m]===k){return true}}return false}function f(k,l){a(k);return e(k,l)}var h;if(g){var b=document.createElement("div");
var j=e(b,"div");h=j?e:f}else{h=i}if(typeof define==="function"&&define.amd){define("matches-selector/matches-selector",[],function(){return h
})}else{window.matchesSelector=h}})(this,Element.prototype);(function(g){var b=g.getComputedStyle;var c=b?function(h){return b(h,null)
}:function(h){return h.currentStyle};function a(h,i){for(var j in i){h[j]=i[j]}return h}function d(h){for(var i in h){return false
}i=null;return true}function f(h){return h.replace(/([A-Z])/g,function(i){return"-"+i.toLowerCase()})}function e(j,k,l){var t=l("transition");
var r=l("transform");var q=t&&r;var m=!!l("perspective");var s={WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"otransitionend",transition:"transitionend"}[t];
var p=["transform","transition","transitionDuration","transitionProperty"];var v=(function(){var w={};for(var x=0,y=p.length;
x<y;x++){var z=p[x];var A=l(z);if(A&&A!==z){w[z]=A}}return w})();function n(w,x){if(!w){return}this.element=w;this.layout=x;this.position={x:0,y:0};
this._create()}a(n.prototype,j.prototype);n.prototype._create=function(){this._transn={ingProperties:{},clean:{},onEnd:{}};this.css({position:"absolute"})
};n.prototype.handleEvent=function(w){var x="on"+w.type;if(this[x]){this[x](w)}};n.prototype.getSize=function(){this.size=k(this.element)
};n.prototype.css=function(y){var w=this.element.style;for(var x in y){var z=v[x]||x;w[z]=y[x]}};n.prototype.getPosition=function(){var C=c(this.element);
var A=this.layout.options;var w=A.isOriginLeft;var z=A.isOriginTop;var D=parseInt(C[w?"left":"right"],10);var E=parseInt(C[z?"top":"bottom"],10);
D=isNaN(D)?0:D;E=isNaN(E)?0:E;var B=this.layout.size;D-=w?B.paddingLeft:B.paddingRight;E-=z?B.paddingTop:B.paddingBottom;this.position.x=D;
this.position.y=E};n.prototype.layoutPosition=function(){var x=this.layout.size;var w=this.layout.options;var y={};if(w.isOriginLeft){y.left=(this.position.x+x.paddingLeft)+"px";
y.right=""}else{y.right=(this.position.x+x.paddingRight)+"px";y.left=""}if(w.isOriginTop){y.top=(this.position.y+x.paddingTop)+"px";
y.bottom=""}else{y.bottom=(this.position.y+x.paddingBottom)+"px";y.top=""}this.css(y);this.emitEvent("layout",[this])};var u=m?function(w,z){return"translate3d("+w+"px, "+z+"px, 0)"
}:function(w,z){return"translate("+w+"px, "+z+"px)"};n.prototype._transitionTo=function(H,I){this.getPosition();var A=this.position.x;
var B=this.position.y;var w=parseInt(H,10);var z=parseInt(I,10);var C=w===this.position.x&&z===this.position.y;this.setPosition(H,I);
if(C&&!this.isTransitioning){this.layoutPosition();return}var F=H-A;var G=I-B;var E={};var D=this.layout.options;F=D.isOriginLeft?F:-F;
G=D.isOriginTop?G:-G;E.transform=u(F,G);this.transition({to:E,onTransitionEnd:{transform:this.layoutPosition},isCleaning:true})
};n.prototype.goTo=function(w,z){this.setPosition(w,z);this.layoutPosition()};n.prototype.moveTo=q?n.prototype._transitionTo:n.prototype.goTo;
n.prototype.setPosition=function(w,z){this.position.x=parseInt(w,10);this.position.y=parseInt(z,10)};n.prototype._nonTransition=function(w){this.css(w.to);
if(w.isCleaning){this._removeStyles(w.to)}for(var x in w.onTransitionEnd){w.onTransitionEnd[x].call(this)}};n.prototype._transition=function(x){if(!parseFloat(this.layout.options.transitionDuration)){this._nonTransition(x);
return}var w=this._transn;for(var z in x.onTransitionEnd){w.onEnd[z]=x.onTransitionEnd[z]}for(z in x.to){w.ingProperties[z]=true;
if(x.isCleaning){w.clean[z]=true}}if(x.from){this.css(x.from);var y=this.element.offsetHeight;y=null}this.enableTransition(x.to);
this.css(x.to);this.isTransitioning=true};var o=r&&(f(r)+",opacity");n.prototype.enableTransition=function(){if(this.isTransitioning){return
}this.css({transitionProperty:o,transitionDuration:this.layout.options.transitionDuration});this.element.addEventListener(s,this,false)
};n.prototype.transition=n.prototype[t?"_transition":"_nonTransition"];n.prototype.onwebkitTransitionEnd=function(w){this.ontransitionend(w)
};n.prototype.onotransitionend=function(w){this.ontransitionend(w)};var i={"-webkit-transform":"transform","-moz-transform":"transform","-o-transform":"transform"};
n.prototype.ontransitionend=function(x){if(x.target!==this.element){return}var w=this._transn;var z=i[x.propertyName]||x.propertyName;
delete w.ingProperties[z];if(d(w.ingProperties)){this.disableTransition()}if(z in w.clean){this.element.style[x.propertyName]="";
delete w.clean[z]}if(z in w.onEnd){var y=w.onEnd[z];y.call(this);delete w.onEnd[z]}this.emitEvent("transitionEnd",[this])};n.prototype.disableTransition=function(){this.removeTransitionStyles();
this.element.removeEventListener(s,this,false);this.isTransitioning=false};n.prototype._removeStyles=function(y){var w={};for(var x in y){w[x]=""
}this.css(w)};var h={transitionProperty:"",transitionDuration:""};n.prototype.removeTransitionStyles=function(){this.css(h)};
n.prototype.removeElem=function(){if(this.element.parentNode){this.element.parentNode.removeChild(this.element)}this.emitEvent("remove",[this])
};n.prototype.remove=function(){if(!t||!parseFloat(this.layout.options.transitionDuration)){this.removeElem();return}var w=this;
this.on("transitionEnd",function(){w.removeElem();return true});this.hide()};n.prototype.reveal=function(){delete this.isHidden;
this.css({display:""});var w=this.layout.options;this.transition({from:w.hiddenStyle,to:w.visibleStyle,isCleaning:true})};n.prototype.hide=function(){this.isHidden=true;
this.css({display:""});var w=this.layout.options;this.transition({from:w.visibleStyle,to:w.hiddenStyle,isCleaning:true,onTransitionEnd:{opacity:function(){if(this.isHidden){this.css({display:"none"})
}}}})};n.prototype.destroy=function(){this.css({position:"",left:"",right:"",top:"",bottom:"",transition:"",transform:""})};return n
}if(typeof define==="function"&&define.amd){define("outlayer/item",["eventEmitter/EventEmitter","get-size/get-size","get-style-property/get-style-property"],e)
}else{g.Outlayer={};g.Outlayer.Item=e(g.EventEmitter,g.getSize,g.getStyleProperty)}})(window);
/*
 * Outlayer v1.2.0
 * the brains and guts of a layout library
 * MIT license
 */
(function(p){var b=p.document;
var a=p.console;var i=p.jQuery;var k=function(){};function c(q,r){for(var s in r){q[s]=r[s]}return q}var l=Object.prototype.toString;
function e(q){return l.call(q)==="[object Array]"}function j(t){var q=[];if(e(t)){q=t}else{if(t&&typeof t.length==="number"){for(var r=0,s=t.length;
r<s;r++){q.push(t[r])}}else{q.push(t)}}return q}var f=(typeof HTMLElement==="object")?function g(q){return q instanceof HTMLElement
}:function h(q){return q&&typeof q==="object"&&q.nodeType===1&&typeof q.nodeName==="string"};var d=Array.prototype.indexOf?function(q,r){return q.indexOf(r)
}:function(q,t){for(var r=0,s=q.length;r<s;r++){if(q[r]===t){return r}}return -1};function n(s,q){var r=d(q,s);if(r!==-1){q.splice(r,1)
}}function o(q){return q.replace(/(.)([A-Z])/g,function(t,r,s){return r+"-"+s}).toLowerCase()}function m(s,q,r,t,x,w){var u=0;
var v={};function y(z,B){if(typeof z==="string"){z=b.querySelector(z)}if(!z||!f(z)){if(a){a.error("Bad "+this.constructor.namespace+" element: "+z)
}return}this.element=z;this.options=c({},this.constructor.defaults);this.option(B);var A=++u;this.element.outlayerGUID=A;v[A]=this;
this._create();if(this.options.isInitLayout){this.layout()}}y.namespace="outlayer";y.Item=w;y.defaults={containerStyle:{position:"relative"},isInitLayout:true,isOriginLeft:true,isOriginTop:true,isResizeBound:true,isResizingContainer:true,transitionDuration:"0.4s",hiddenStyle:{opacity:0,transform:"scale(0.001)"},visibleStyle:{opacity:1,transform:"scale(1)"}};
c(y.prototype,r.prototype);y.prototype.option=function(z){c(this.options,z)};y.prototype._create=function(){this.reloadItems();
this.stamps=[];this.stamp(this.options.stamp);c(this.element.style,this.options.containerStyle);if(this.options.isResizeBound){this.bindResize()
}};y.prototype.reloadItems=function(){this.items=this._itemize(this.element.children)};y.prototype._itemize=function(A){var E=this._filterFindItemElements(A);
var D=this.constructor.Item;var F=[];for(var B=0,G=E.length;B<G;B++){var z=E[B];var C=new D(z,this);F.push(C)}return F};y.prototype._filterFindItemElements=function(B){B=j(B);
var E=this.options.itemSelector;var D=[];for(var C=0,H=B.length;C<H;C++){var A=B[C];if(!f(A)){continue}if(E){if(x(A,E)){D.push(A)
}var z=A.querySelectorAll(E);for(var F=0,G=z.length;F<G;F++){D.push(z[F])}}else{D.push(A)}}return D};y.prototype.getItemElements=function(){var z=[];
for(var A=0,B=this.items.length;A<B;A++){z.push(this.items[A].element)}return z};y.prototype.layout=function(){this._resetLayout();
this._manageStamps();var z=this.options.isLayoutInstant!==undefined?this.options.isLayoutInstant:!this._isLayoutInited;this.layoutItems(this.items,z);
this._isLayoutInited=true};y.prototype._init=y.prototype.layout;y.prototype._resetLayout=function(){this.getSize()};y.prototype.getSize=function(){this.size=t(this.element)
};y.prototype._getMeasurement=function(A,C){var B=this.options[A];var z;if(!B){this[A]=0}else{if(typeof B==="string"){z=this.element.querySelector(B)
}else{if(f(B)){z=B}}this[A]=z?t(z)[C]:B}};y.prototype.layoutItems=function(A,z){A=this._getItemsForLayout(A);this._layoutItems(A,z);
this._postLayout()};y.prototype._getItemsForLayout=function(B){var C=[];for(var z=0,D=B.length;z<D;z++){var A=B[z];if(!A.isIgnored){C.push(A)
}}return C};y.prototype._layoutItems=function(D,B){var z=this;function F(){z.emitEvent("layoutComplete",[z,D])}if(!D||!D.length){F();
return}this._itemsOn(D,"layout",F);var H=[];for(var A=0,E=D.length;A<E;A++){var C=D[A];var G=this._getItemLayoutPosition(C);G.item=C;
G.isInstant=B||C.isLayoutInstant;H.push(G)}this._processLayoutQueue(H)};y.prototype._getItemLayoutPosition=function(){return{x:0,y:0}
};y.prototype._processLayoutQueue=function(C){for(var z=0,A=C.length;z<A;z++){var B=C[z];this._positionItem(B.item,B.x,B.y,B.isInstant);
if(B.width){B.item.element.setAttribute("style",B.item.element.getAttribute("style")+"width:"+B.width+"px")}}};y.prototype._positionItem=function(A,B,C,z){if(z){A.goTo(B,C)
}else{A.moveTo(B,C)}};y.prototype._postLayout=function(){this.resizeContainer()};y.prototype.resizeContainer=function(){if(!this.options.isResizingContainer){return
}var z=this._getContainerSize();if(z){this._setContainerMeasure(z.width,true);this._setContainerMeasure(z.height,false)}};y.prototype._getContainerSize=k;
y.prototype._setContainerMeasure=function(B,A){if(B===undefined){return}var z=this.size;if(z.isBorderBox){B+=A?z.paddingLeft+z.paddingRight+z.borderLeftWidth+z.borderRightWidth:z.paddingBottom+z.paddingTop+z.borderTopWidth+z.borderBottomWidth
}B=Math.max(B,0);this.element.style[A?"width":"height"]=B+"px"};y.prototype._itemsOn=function(G,D,A){var C=0;var B=G.length;var z=this;
function I(){C++;if(C===B){A.call(z)}return true}for(var E=0,H=G.length;E<H;E++){var F=G[E];F.on(D,I)}};y.prototype.ignore=function(z){var A=this.getItem(z);
if(A){A.isIgnored=true}};y.prototype.unignore=function(z){var A=this.getItem(z);if(A){delete A.isIgnored}};y.prototype.stamp=function(A){A=this._find(A);
if(!A){return}this.stamps=this.stamps.concat(A);for(var B=0,C=A.length;B<C;B++){var z=A[B];this.ignore(z)}};y.prototype.unstamp=function(A){A=this._find(A);
if(!A){return}for(var B=0,C=A.length;B<C;B++){var z=A[B];n(z,this.stamps);this.unignore(z)}};y.prototype._find=function(z){if(!z){return
}if(typeof z==="string"){z=this.element.querySelectorAll(z)}z=j(z);return z};y.prototype._manageStamps=function(){if(!this.stamps||!this.stamps.length){return
}this._getBoundingRect();for(var z=0,A=this.stamps.length;z<A;z++){var B=this.stamps[z];this._manageStamp(B)}};y.prototype._getBoundingRect=function(){var z=this.element.getBoundingClientRect();
var A=this.size;this._boundingRect={left:z.left+A.paddingLeft+A.borderLeftWidth,top:z.top+A.paddingTop+A.borderTopWidth,right:z.right-(A.paddingRight+A.borderRightWidth),bottom:z.bottom-(A.paddingBottom+A.borderBottomWidth)}
};y.prototype._manageStamp=k;y.prototype._getElementOffset=function(A){var z=A.getBoundingClientRect();var D=this._boundingRect;
var C=t(A);var B={left:z.left-D.left-C.marginLeft,top:z.top-D.top-C.marginTop,right:D.right-z.right-C.marginRight,bottom:D.bottom-z.bottom-C.marginBottom};
return B};y.prototype.handleEvent=function(z){var A="on"+z.type;if(this[A]){this[A](z)}};y.prototype.bindResize=function(){if(this.isResizeBound){return
}s.bind(p,"resize",this);this.isResizeBound=true};y.prototype.unbindResize=function(){if(this.isResizeBound){s.unbind(p,"resize",this)
}this.isResizeBound=false};y.prototype.onresize=function(){if(this.resizeTimeout){clearTimeout(this.resizeTimeout)}var z=this;
function A(){z.resize();delete z.resizeTimeout}this.resizeTimeout=setTimeout(A,100)};y.prototype.resize=function(){if(!this.isResizeBound||!this.needsResizeLayout()){return
}this.layout()};y.prototype.needsResizeLayout=function(){var A=t(this.element);var z=this.size&&A;return z&&A.innerWidth!==this.size.innerWidth
};y.prototype.addItems=function(z){var A=this._itemize(z);if(A.length){this.items=this.items.concat(A)}return A};y.prototype.appended=function(z){var A=this.addItems(z);
if(!A.length){return}this.layoutItems(A,true);this.reveal(A)};y.prototype.prepended=function(z){var A=this._itemize(z);if(!A.length){return
}var B=this.items.slice(0);this.items=A.concat(B);this._resetLayout();this._manageStamps();this.layoutItems(A,true);this.reveal(A);
this.layoutItems(B)};y.prototype.reveal=function(B){var C=B&&B.length;if(!C){return}for(var z=0;z<C;z++){var A=B[z];A.reveal()
}};y.prototype.hide=function(B){var C=B&&B.length;if(!C){return}for(var z=0;z<C;z++){var A=B[z];A.hide()}};y.prototype.getItem=function(z){for(var A=0,C=this.items.length;
A<C;A++){var B=this.items[A];if(B.element===z){return B}}};y.prototype.getItems=function(A){if(!A||!A.length){return}var D=[];
for(var B=0,E=A.length;B<E;B++){var z=A[B];var C=this.getItem(z);if(C){D.push(C)}}return D};y.prototype.remove=function(z){z=j(z);
var D=this.getItems(z);if(!D||!D.length){return}this._itemsOn(D,"remove",function(){this.emitEvent("removeComplete",[this,D])
});for(var A=0,C=D.length;A<C;A++){var B=D[A];B.remove();n(B,this.items)}};y.prototype.destroy=function(){var C=this.element.style;
C.height="";C.position="";C.width="";for(var z=0,B=this.items.length;z<B;z++){var A=this.items[z];A.destroy()}this.unbindResize();
delete this.element.outlayerGUID;if(i){i.removeData(this.element,this.constructor.namespace)}};y.data=function(z){var A=z&&z.outlayerGUID;
return A&&v[A]};y.create=function(B,C){function z(){y.apply(this,arguments)}if(Object.create){z.prototype=Object.create(y.prototype)
}else{c(z.prototype,y.prototype)}z.prototype.constructor=z;z.defaults=c({},y.defaults);c(z.defaults,C);z.prototype.settings={};
z.namespace=B;z.data=y.data;z.Item=function A(){w.apply(this,arguments)};z.Item.prototype=new w();q(function(){var E=o(B);var H=b.querySelectorAll(".js-"+E);
var F="data-"+E+"-options";for(var J=0,L=H.length;J<L;J++){var G=H[J];var D=G.getAttribute(F);var M;try{M=D&&JSON.parse(D)}catch(I){if(a){a.error("Error parsing "+F+" on "+G.nodeName.toLowerCase()+(G.id?"#"+G.id:"")+": "+I)
}continue}var K=new z(G,M);if(i){i.data(G,B,K)}}});if(i&&i.bridget){i.bridget(B,z)}return z};y.Item=w;return y}if(typeof define==="function"&&define.amd){define("outlayer/outlayer",["eventie/eventie","doc-ready/doc-ready","eventEmitter/EventEmitter","get-size/get-size","matches-selector/matches-selector","./item"],m)
}else{p.Outlayer=m(p.eventie,p.docReady,p.EventEmitter,p.getSize,p.matchesSelector,p.Outlayer.Item)}})(window);
/*
 * Masonry v3.1.5
 * Cascading grid layout library
 * http://masonry.desandro.com
 ...
 * by David DeSandro
 */
(function(c){var a=Array.prototype.indexOf?function(d,e){return d.indexOf(e)
}:function(f,h){for(var d=0,g=f.length;d<g;d++){var e=f[d];if(e===h){return d}}return -1};function b(f,d){var e=f.create("masonry");
e.prototype._resetLayout=function(){this.getSize();this._getMeasurement("columnWidth","outerWidth");this._getMeasurement("gutter","outerWidth");
this.measureColumns();var g=this.cols;this.colYs=[];while(g--){this.colYs.push(0)}this.maxY=0};e.prototype.measureColumns=function(){this.getContainerWidth();
if(!this.columnWidth){var g=this.items[0];var h=g&&g.element;this.columnWidth=h&&d(h).outerWidth||this.containerWidth}this.columnWidth+=this.gutter;
this.cols=Math.floor((this.containerWidth+this.gutter)/this.columnWidth);this.cols=Math.max(this.cols,1)};e.prototype.getContainerWidth=function(){var g=this.options.isFitWidth?this.element.parentNode:this.element;
var h=d(g);this.containerWidth=h&&h.innerWidth};e.prototype._getItemLayoutPosition=function(k){k.getSize();var o=k.size.outerWidth%this.columnWidth;
var l=o&&o<1?"round":"ceil";var h=Math[l](k.size.outerWidth/this.columnWidth);h=Math.min(h,this.cols);var g=this._getColGroup(h);
var m=Math.min.apply(Math,g);var s=a(g,m);var n={x:this.columnWidth*s,y:m,};var r=h*this.columnWidth;if(h>1){n.width=r-(r*h*0.025)
}var p=m+k.size.outerHeight;var q=this.cols+1-g.length;for(var j=0;j<q;j++){this.colYs[s+j]=p}return n};e.prototype._getColGroup=function(h){if(h<2){return this.colYs
}var g=[];var k=this.cols+1-h;for(var l=0;l<k;l++){var j=this.colYs.slice(l,l+h);g[l]=Math.max.apply(Math,j)}return g};e.prototype._manageStamp=function(n){var p=d(n);
var m=this._getElementOffset(n);var h=this.options.isOriginLeft?m.left:m.right;var l=h+p.outerWidth;var g=Math.floor(h/this.columnWidth);
g=Math.max(0,g);var k=Math.floor(l/this.columnWidth);k-=l%this.columnWidth?0:1;k=Math.min(this.cols-1,k);var o=(this.options.isOriginTop?m.top:m.bottom)+p.outerHeight;
for(var j=g;j<=k;j++){this.colYs[j]=Math.max(o,this.colYs[j])}};e.prototype._getContainerSize=function(){this.maxY=Math.max.apply(Math,this.colYs);
var g={height:this.maxY};if(this.options.isFitWidth){g.width=this._getContainerFitWidth()}return g};e.prototype._getContainerFitWidth=function(){var h=0;
var g=this.cols;while(--g){if(this.colYs[g]!==0){break}h++}return(this.cols-h)*this.columnWidth-this.gutter};e.prototype.needsResizeLayout=function(){var g=this.containerWidth;
this.getContainerWidth();return g!==this.containerWidth};return e}if(typeof define==="function"&&define.amd){define(["outlayer/outlayer","get-size/get-size"],b)
}else{c.Masonry=b(c.Outlayer,c.getSize)}})(window);
//
...
}else{q.css("top","")}if(E){A.scrollTo(0,E)}},performCloseDialog:function(D,H){var F=D.attr("id");if(typeof A.Hammer!=="undefined"){var E=i.hammerTime;
E.off("tap",l);E.destroy&&E.destroy();delete i.hammerTime}else{D.unbind("click")}if(o===1){q.unbind("keydown")}if(!D.hasClass("ngdialog-closing")){o-=1
}var G=D.data("$ngDialogPreviousFocus");if(G){G.focus()}x.$broadcast("ngDialog.closing",D);o=o<0?0:o;if(d){i.$destroy();D.unbind(c).bind(c,function(){D.remove();
if(o===0){u.removeClass("ngdialog-open");q.removeClass("ngdialog-open");q.find(".recipe-subNav").show();B.resetBodyPadding();
B.resetBodyTopPosition()}x.$broadcast("ngDialog.closed",D)}).addClass("ngdialog-closing")}else{i.$destroy();D.remove();if(o===0){u.removeClass("ngdialog-open");
q.removeClass("ngdialog-open");q.find(".recipe-subNav").show();B.resetBodyPadding();B.resetBodyTopPosition()}x.$broadcast("ngDialog.closed",D)
}if(n[F]){n[F].resolve({id:F,value:H,$dialog:D,remainingDialogs:o});delete n[F]}},closeDialog:function(D,G){var E=D.data("$ngDialogPreCloseCallback");
if(E&&b.isFunction(E)){var F=E.call(D,G);if(b.isObject(F)){if(F.closePromise){F.closePromise.then(function(){B.performCloseDialog(D,G)
...
var D,E;w.when(G(I.template||I.templateUrl)).then(function(P){y.put(I.template||I.templateUrl,P);if(I.showClose){P+='<div class="ngdialog-close"></div>'
}K.$result=D=a('<div id="ngdialog'+p+'" class="ngdialog"></div>');D.html((I.overlay?'<div class="ngdialog-overlay"></div><div class="ngdialog-content" role="document">'+P+"</div>":'<div class="ngdialog-content" role="document">'+P+"</div>"));
D.data("$ngDialogOptions",I);if(I.data&&b.isString(I.data)){var M=I.data.replace(/^\s*/,"")[0];i.ngDialogData=(M==="{"||M==="[")?b.fromJson(I.data):I.data
}else{if(I.data&&b.isObject(I.data)){i.ngDialogData=I.data}}if(I.controller&&(b.isString(I.controller)||b.isArray(I.controller)||b.isFunction(I.controller))){var L=s(I.controller,{$scope:i,$element:D});
D.data("$ngDialogControllerController",L)}if(I.className){D.addClass(I.className)}if(I.appendTo&&b.isString(I.appendTo)){E=b.element(document.querySelector(I.appendTo))
}else{E=q}B.applyAriaAttributes(D,I);if(I.preCloseCallback){var O;if(b.isFunction(I.preCloseCallback)){O=I.preCloseCallback}else{if(b.isString(I.preCloseCallback)){if(i){if(b.isFunction(i[I.preCloseCallback])){O=i[I.preCloseCallback]
}else{if(i.$parent&&b.isFunction(i.$parent[I.preCloseCallback])){O=i.$parent[I.preCloseCallback]}else{if(x&&b.isFunction(x[I.preCloseCallback])){O=x[I.preCloseCallback]
}}}}}}if(O){D.data("$ngDialogPreCloseCallback",O)}}i.closeThisDialog=function(Q){B.closeDialog(D,Q)};z(function(){var Q=document.querySelectorAll(".ngdialog");
B.deactivateAll(Q);r(D)(i);var S=A.innerWidth-q.prop("clientWidth");B.setBodyTopPosition();q.addClass("ngdialog-open");u.addClass("ngdialog-open");
q.find(".recipe-subNav").hide();var R=S-(A.innerWidth-q.prop("clientWidth"));if(R>0){B.setBodyPadding(R)}E.append(D);B.activate(D);
...
})}if(I.preserveFocus){D.data("$ngDialogPreviousFocus",document.activeElement)}l=function(Q){var S=I.closeByDocument?a(Q.target).hasClass("ngdialog-overlay"):false;
var R=a(Q.target).hasClass("ngdialog-close");if(S||R){C.close(D.attr("id"),R?"$closeButton":"$document")}};if(typeof A.Hammer!=="undefined"){var N=i.hammerTime=A.Hammer(D[0]);
N.on("tap",l)}else{D.bind("click",l)}o+=1;return C});return{id:"ngdialog"+p,closePromise:F.promise,close:function(L){B.closeDialog(D,L)
...
}else{C.closeAll(F)}return C},closeAll:function(G){var D=document.querySelectorAll(".ngdialog");for(var F=D.length;F>-1;F--){var E=D[F];
B.closeDialog(a(E),G)}},getDefaults:function(){return m}};return C}]});h.directive("ngDialog",["ngDialog",function(l){return{restrict:"A",scope:{ngDialogScope:"="},link:function(o,n,m){n.on("click",function(q){q.preventDefault();
...
//
angular.module("allrecipes",["LocalStorageModule","ngAnimate","ngSanitize","ngCookies","ngResource","ngRoute","angularFileUpload","wu.masonry","ipCookie","ngDialog"]);
if(typeof adManager!=="undefined"){angular.module("allrecipes").value("adManager",adManager)}if(typeof adService!=="undefined"){angular.module("allrecipes").value("adService",adService)
}angular.module("allrecipes").config(["$routeProvider","$locationProvider","$httpProvider",function(c,b,a){var d=window.history&&window.history.pushState?true:false;
b.html5Mode({enabled:d,requireBase:false});a.defaults.headers.common={"X-Requested-With":"XMLHttpRequest"};c.when("/cook/my/",{templateUrl:"/templates/profile/private/favorites.html",controller:"ar_controllers_profile"}).when("/cook/my/collections/",{templateUrl:"/templates/profile/private/favorites.html",controller:"ar_controllers_profile"}).when("/cook/my/favorites/",{templateUrl:"/templates/profile/private/favorites.html",controller:"ar_controllers_profile"}).when("/cook/my/favorite-recipess/",{templateUrl:"/templates/profile/private/favorites.html",controller:"ar_controllers_profile"}).when("/cook/my/friends/",{templateUrl:"/templates/profile/private/friends.html",controller:"ar_controllers_profile"}).when("/cook/my/following/",{templateUrl:"/templates/profile/private/friends.html",controller:"ar_controllers_profile"}).when("/cook/my/followers/",{templateUrl:"/templates/profile/private/friends.html",controller:"ar_controllers_profile"}).when("/cook/my/made-it/",{templateUrl:"/templates/profile/private/madeit.html",controller:"ar_controllers_profile"}).when("/cook/my/photos/",{templateUrl:"/templates/profile/private/photos.html",controller:"ar_controllers_profile"}).when("/cook/my/recipes/",{templateUrl:"/templates/profile/private/recipes.html",controller:"ar_controllers_profile"}).when("/cook/my/reviews/",{templateUrl:"/templates/profile/private/reviews.html",controller:"ar_controllers_profile"}).when("/cook/my/findfriends/",{templateUrl:"/templates/profile/private/friends.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/",{templateUrl:"/templates/profile/public/favorites.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/collections/",{templateUrl:"/templates/profile/public/favorites.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/made-it/",{templateUrl:"/templates/profile/public/madeit.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/favorites/",{templateUrl:"/templates/profile/public/favorites.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/favorite-recipes/",{templateUrl:"/templates/profile/public/favorites.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/following/",{templateUrl:"/templates/profile/public/following.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/followers/",{templateUrl:"/templates/profile/public/followers.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/recipes/",{templateUrl:"/templates/profile/public/recipes.html",controller:"ar_controllers_profile"}).when("/cook/:cookId/reviews/",{templateUrl:"/templates/profile/public/reviews.html",controller:"ar_controllers_profile"}).otherwise({redirectTo:function(){window.location="/error/notfound/"
}})}]);
//
...
};a.madeItUndo=function(b){return{recipeId:b}};a.followPost=function(b,c){return{userID:b,deferredActionName:c}}})(ar.models=ar.models||{});
//
// source/js/foresee/foresee.js
//
var AR=AR||{};AR.Account=AR.Account||{};(function(a){a.VisitorId=function(){var c="";if(window.adConfiguration&&window.adConfiguration.pagetargets){for(var b=0;
b<window.adConfiguration.pagetargets.length;b++){if(window.adConfiguration.pagetargets[b].key=="oid"){c=window.adConfiguration.pagetargets[b].value;
break}}}return c};a.VisitorStatus=function(){var c="";if(window.adConfiguration&&window.adConfiguration.pagetargets){for(var b=0;
b<window.adConfiguration.pagetargets.length;b++){if(window.adConfiguration.pagetargets[b].key=="status"){c=window.adConfiguration.pagetargets[b].value;
break}}}return c}})(AR.Account.VisitorInfo=AR.Account.VisitorInfo||{});(function(){var b=0;var c=100;var a=function(){if(!window.ArAdsHasInterstitial){var d=window.Toggles.AdTest===true;
var g=RegExp("[?&]adtest=true").exec(window.location.search)||(d===true);if(!g&&window.location.protocol!="https:"){if(document.getElementById("foresee-js")){return
}var e=document.createElement("script");e.type="text/javascript";e.async=true;e.src="//images.media-allrecipes.com/ar/foresee/v0.5/foresee-trigger.js";
e.id="foresee-js";var h=document.getElementsByTagName("script")[0];h.parentNode.insertBefore(e,h);var f=setInterval(function(){if(window.FSR&&window.FSR.CPPS){window.FSR.CPPS.set("ovid",AR.Account.VisitorInfo.VisitorId());
...
//
// source/angular/controllers/popupCtrl.js
//
angular.module("allrecipes").controller("ar_controllers_popup",["$scope","$window",function(a,b){a.message="My message goes here";
a.showAlert=function(c){b.alert(c)};a.showConfirm=function(c){if(event){event.preventDefault()}a.isOkay=b.confirm(c)};a.showConfirmAndRedirect=function(c,d){if(typeof event!=="undefined"){event.preventDefault()
}a.isOkay=b.confirm(c);if(a.isOkay){b.location=d}}}]);
//
...
//
"use strict";angular.module("allrecipes").controller("ar_controllers_footerLinks",["$scope","$window",function(a,b){a.arSites=[{name:"Argentina",url:"allrecipes.com.ar"},{name:"Australia & New Zealand",url:"allrecipes.com.au"},{name:"Brazil",url:"allrecipes.com.br"},{name:"Canada",url:"allrecipes.com?country=CA"},{name:"China",url:"allrecipes.com.cn"},{name:"France",url:"allrecipes.fr"},{name:"Germany",url:"de.allrecipes.com"},{name:"India",url:"allrecipes.co.in"},{name:"Italy",url:"allrecipes.it"},{name:"Korea",url:"allrecipes.kr"},{name:"Mexico",url:"allrecipes.com.mx"},{name:"Netherlands",url:"allrecipes.nl"},{name:"Poland",url:"allrecipes.pl"},{name:"Quebec",url:"qc.allrecipes.ca"},{name:"Russia",url:"allrecipes.ru"},{name:"SE Asia",url:"allrecipes.asia"},{name:"United Kingdom & Ireland",url:"allrecipes.co.uk"},{name:"United States",url:"allrecipes.com"}];
a.changeSite=function(){b.location.href="http://"+a.selectedItem.url}}]);
//
...
//
// source/angular/controllers/newsletterSliderCtrl.js
//
angular.module("allrecipes").controller("ar_controllers_newsletter_slider",["$scope","$attrs","ipCookie","arLogin","ar_services_newsletter","ar_services_environment",function(c,a,b,f,e,d){c.autoSubscribeCookieName="ARSubscribeNewsletterId";
c.isHidden=true;c.isLoggedIn=f.isLoggedIn();c.decideSliderVisibility=function(h){c.newsletterId=b(c.autoSubscribeCookieName);
if(c.newsletterId){if(c.isLoggedIn){c.subscribeNewsletter()}else{c.isHidden=false}}else{c.newsletterId=h||a.newsletterid;if(c.newsletterId){if(c.isLoggedIn){c.checkNewsletterSubscribed()
}else{c.isHidden=false}}}b.remove(c.autoSubscribeCookieName)};c.attemptSubscribeNewsletter=function(){if(!c.isLoggedIn){b(c.autoSubscribeCookieName,c.newsletterId)
}f.ensureUserIsLoggedIn(c.subscribeNewsletter)(c.isLoggedIn)};c.close=function(i,j){var h=d.baseDomain;b(i,true,{domain:h,path:"/",expires:j,expirationUnit:"days"});
g()};c.subscribeNewsletterSuccess=null;c.subscribeNewsletter=function(){var i=function(){c.$emit("notify","Newsletter subscription added. Thank you!",null,"success",null,null,g)
};var h=function(){c.$emit("notify","Sorry, the newsletter subscription could not be added. Please try again later.",null,"failure",null,null,g)
};e.addSubscription({newsletterId:c.newsletterId}).$promise.then(function(j){if(j.data.subscriptionSuccess){c.subscribeNewsletterSuccess=true
}else{c.subscribeNewsletterSuccess=false}},function(j){c.subscribeNewsletterSuccess=false})["finally"](function(){c.isHidden=false;
if(c.subscribeNewsletterSuccess){i()}else{h()}})};c.checkNewsletterSubscribed=function(){e.getSubscriptions().$promise.then(function(i){if(i.data.newsletterIds){var h=parseInt(c.newsletterId,10);
c.isHidden=(i.data.newsletterIds.indexOf(h)!=-1)}else{c.isHidden=false}},function(h){c.isHidden=false})};c.decideSliderVisibility();
var g=function(){c.isHidden=true}}]);
//
...
if(d){d.focus()}})}}]);
//
// source/angular/services/ContentProvider-Provider.js
//
"use strict";angular.module("allrecipes").factory("ar_services_contentprovider",["ar_services_environment","$resource","ar_services_token",function(b,a,c){var d=b.url+"v1/contentprovider";
return a(d,{sourceid:"@sourceid"},{getProvider:{url:b.url+"v1/contentprovider/provider",method:"GET",data:"@sourceid",headers:{Authorization:c.token}},getZone:{url:b.url+"v1/contentprovider/zone/:id",method:"GET",isArray:false,headers:{Authorization:c.token}}})
}]);
//
...
//
"use strict";angular.module("allrecipes").constant("SEARCH_CONSTANTS",{MaxAllowedIngredients:4,MaxIngredientLength:50,EnterKey:13,TabKey:9,BackspaceKey:8,DOM_Element:1,InclIngredHintText:"Include ingredients",ExclIngredHintText:"Exclude ingredients"});
//
// source/angular/controllers/searchCtrl.js
//
angular.module("allrecipes").controller("ar_controllers_search",["$scope","$window","$document","$timeout","ar_services_url","SEARCH_CONSTANTS",function(b,d,a,c,e,o){b.search={keywords:"",ingredientsInclude:[],ingredientsExclude:[],};
b.includeIngredient=null;b.excludeIngredient=null;var r=function(){b.includeIngHitMax=(b.search.ingredientsInclude.length>=o.MaxAllowedIngredients);
b.excludeIngHitMax=(b.search.ingredientsExclude.length>=o.MaxAllowedIngredients);b.includeIngPlaceholderText=(b.search.ingredientsInclude.length>0?null:o.InclIngredHintText);
b.excludeIngPlaceholderText=(b.search.ingredientsExclude.length>0?null:o.ExclIngredHintText)};b.initialize=function(){var x=function(B){var A=d.location.search.substring(1);
var C=A.split("&");for(var y=0;y<C.length;y++){var z=C[y].split("=");if(decodeURIComponent(z[0])==B){return decodeURIComponent(z[1])
}}return undefined};var w=function(y){var z=x(y);return z?z:""};b.search.keywords=w("wt");var v=function(y){var z=x(y);return z?z.split(","):[]
};b.search.ingredientsInclude=v("ingIncl");b.search.ingredientsExclude=v("ingExcl");r()};b.initialize();b.footerAd=angular.element("#div-gpt-ad-footer");
b.hideAds=function(){b.footerAd.hide()};b.showAds=function(){b.footerAd.show()};b.isEnterKey=function(v){return(v.which===o.EnterKey)
};b.isTabKey=function(v){return(v.which===o.TabKey)};b.isBackspaceKey=function(v){return(v.which===o.BackspaceKey)};b.addIngredientInclude=function(w){if(!b.includeIngredient){return
}var v=b.includeIngredient.trim();if(v.length>0){if(b.includeIngHitMax){b.$emit("notify","You can include up to "+o.MaxAllowedIngredients+" ingredients.");
b.includeIngredient=null;return}v=v.substring(0,o.MaxIngredientLength);if(!t(v,b.search.ingredientsInclude)){p(w);b.search.ingredientsInclude.push(v)
}}b.includeIngredient=null;r();c(function(){if(w){s()}})};b.addIngredientExclude=function(w){if(!b.excludeIngredient){return}var v=b.excludeIngredient.trim();
if(v.length>0){if(b.excludeIngHitMax){b.$emit("notify","You can exclude up to "+o.MaxAllowedIngredients+" ingredients.");b.excludeIngredient=null;
return}v=v.substring(0,o.MaxIngredientLength);if(!t(v,b.search.ingredientsExclude)){p(w);b.search.ingredientsExclude.push(v)}}b.excludeIngredient=null;
r();c(function(){if(w){s()}})};b.removeLastIngredientInclude=function(w){if(!b.includeIngredient){var v=b.search.ingredientsInclude;
if(v.length>0){b.removeIngredientInclude(v[v.length-1],w);c(function(){if(w){s()}})}}};b.removeIngredientInclude=function(w,y){for(var v=0;
v<b.search.ingredientsInclude.length;v++){var x=b.search.ingredientsInclude[v];if(x===w){p(y);b.search.ingredientsInclude.splice(v,1);
break}}r();c(function(){if(y){n()}})};b.removeLastIngredientExclude=function(w){if(!b.excludeIngredient){var v=b.search.ingredientsExclude;
if(v.length>0){b.removeIngredientExclude(v[v.length-1],w);c(function(){if(w){s()}})}}};b.removeIngredientExclude=function(w,y){for(var v=0;
v<b.search.ingredientsExclude.length;v++){var x=b.search.ingredientsExclude[v];if(x===w){p(y);b.search.ingredientsExclude.splice(v,1);
break}}r();c(function(){if(y){n()}})};b.performSearch=function(){if(!b.includeIngHitMax){b.addIngredientInclude()}b.includeIngredient=null;
if(!b.excludeIngHitMax){b.addIngredientExclude()}b.excludeIngredient=null;var x=b.search.keywords;var w=b.search.ingredientsInclude;
var v=b.search.ingredientsExclude;if(x.length==0&&w.length==0&&v.length==0){b.$emit("notify","Oops! Enter a recipe or ingredient to search for.",null,"failure")
}else{var y=e.getSearchUrl(x,w,v);d.location.href=y}};b.mouseDown=function(w){p(w);var v=b.currentDragElement;b.horizDisplacement=w.clientX-(v.style.left?parseInt(v.style.left):0);
d.addEventListener("mousemove",m,false);a.bind("mouseup",b.mouseUp);w.preventDefault()};b.mouseUp=function(v){d.removeEventListener("mousemove",m,false);
a.unbind("mouseup",b.mouseUp)};b.touchStart=function(w){p(w);var v=b.currentDragElement;b.horizDisplacement=w.originalEvent.targetTouches[0].clientX-(v.style.left?parseInt(v.style.left):0);
d.addEventListener("touchmove",u,false);w.preventDefault()};b.touchEnd=function(v){d.removeEventListener("touchmove",u,false)
};var t=function(x,w){for(var v=0;v<w.length;++v){if(w[v]===x){return true}}return false};var p=function(z){if(z){var y=z.target||z.srcElement;
var A=b.scrollableClassHeirarchy.draggableElement;var w=b.scrollableClassHeirarchy.outermostWrapper;var v=b.scrollableClassHeirarchy.clippingFrame;
var x=k(y,A,w);q(x,A,v)}};var q=function(x,w,v){b.currentDragElement=j(x,w);b.currentClippingFrame=j(x,v)};var j=function(w,v){var x=w;
while(x&&(!x.hasAttribute("class")||x.attributes["class"].value.split(" ").indexOf(v)===-1)){x=x.parentElement}return x};var k=function(x,y,v){var w=j(x,v);
var z=f(w,y);return z};var f=function(w,v){var z=[];z.push(w);while(z.length>0){var x=z.shift();if(x.hasAttribute("class")&&x.attributes["class"].value.split(" ").indexOf(v)!==-1){return x
}if(!x.hasChildNodes){continue}for(var y=0;y<x.childNodes.length;++y){if(x.childNodes[y].nodeType==o.DOM_Element){z.push(x.childNodes[y])
}}}return};var m=function(v){i(v.clientX)};var u=function(v){i(v.targetTouches[0].clientX);v.stopPropagation();v.preventDefault()
};var i=function(v){h(v);g();n()};var h=function(w){var v=b.currentDragElement;v.style.top="0px";v.style.left=(w-b.horizDisplacement)+"px"
};var g=function(){var x=l();var v=(x.clippingRect.left>x.dragRect.left?x.clippingRect.left-x.dragRect.left:0);var w=(x.dragRect.right>x.clippingRect.right?x.dragRect.right-x.clippingRect.right:0);
x.dragElement.style.clip="rect(0px,"+(x.dragRect.right-x.dragRect.left+1-w)+"px,"+(x.dragRect.bottom-x.dragRect.top+1)+"px,"+v+"px)"
};var n=function(){var x=l();var w=x.dragRect.right-x.dragRect.left;var v=x.clippingRect.right-x.clippingRect.left;var y=null;
if(w<=v){y=v-w}else{if(x.dragRect.right<x.clippingRect.right){y=v-w}else{if(x.dragRect.left>x.clippingRect.left){y=0}}}if(y!=null){x.dragElement.style.left=y+"px"
}};var s=function(){var x=l();var w=x.dragRect.right-x.dragRect.left;var v=x.clippingRect.right-x.clippingRect.left;if(w>v){var y=v-w;
x.dragElement.style.left=y+"px"}};var l=function(){return{dragElement:b.currentDragElement,dragRect:b.currentDragElement.getBoundingClientRect(),clippingElement:b.currentClippingFrame,clippingRect:b.currentClippingFrame.getBoundingClientRect()}
}}]);
//
// source/angular/directives/loadingIndicator.js
//
"use strict";angular.module("allrecipes").directive("loadingIndicator",function(){return{restrict:"A",templateUrl:"/templates/loadingindicator.html",terminal:true,replace:false,link:function(c,a){var b=a.find("div").eq(0);
b.hide();c.$on("loadingindicator_show",function(){b.show()});c.$on("loadingindicator_hide",function(){b.hide()})}}});
//
...
}return f},responseError:function(f){if(f&&f.status=="401"&&f.config.headers.Authorization.length>8){var e="";if(f.config.data.deferredActionName){e=((window.location.href.indexOf("?")<0)?"?":"&")+"deferred="+JSON.stringify(f.config.data)
}d.goToSignInPage(encodeURIComponent(window.location.href+e))}else{c.$emit("notify","Oops! There was a problem. Try again in just a moment.",null,"failure")
}c.$broadcast("loadingindicator_hide");return b.reject(f)}}}]);
//
// source/angular/directives/notification.js
//
"use strict";angular.module("allrecipes").directive("arNotification",["$compile","$timeout",function(a,b){return{restrict:"E",template:"<div id='msg-toolbar' class='notification hidden'><span class='notification__message'></span></div>",link:function(k,h,d,f){k.$on("notify",function(s,v,x,y,r,p,q,u,t,w){k.afterNotification=q;
if(r){c(r,p)}else{j()}g(v,y,x,u,t,w)});var m;k.$on("cancelNotification",function(){b.cancel(m);i()});var g=function(r,u,t,q,p,s){n(r);
o(u);if(q){e(q,p,s)}l();m=b(function(){i()},t!=null?t:3000)};var n=function(p){h.find("span").text(p)};var e=function(s,r,u){var q="<a href='/collections/"+r+"'>"+s+"</a>";
var t="";if(u>0){t=" + <a href='/cook/my/'>"+u+" more</a>"}var p=q+t;h.find("span").append(p);h.addClass("notification-for-collections");
a(h)(k)};var o=function(p){if(p==="success"){h.removeClass("failure--message");h.addClass("success--message")}if(p==="failure"){h.removeClass("success--message");
h.addClass("failure--message")}if(p==="progress"){h.removeClass("failure--message");h.removeClass("success--message")}};var l=function(){h.removeClass("hidden");
h.addClass("visible")};var i=function(){h.removeClass("visible");h.addClass("hidden");j();if(k.afterNotification){k.afterNotification()
}};var c=function(r,p){j();var q;if(p){q="<a href='javascript:void(0)' class='btns-one-small btn-made-it' data-ng-click='"+p+"()'>"+r+"</a>"
}else{q='<a href="javascript:void(0)" class="btns-one-small btn-made-it">'+r+"</a>"}h.find("span").after(q);h.addClass("notification-with-button");
a(h)(k)};var j=function(){h.find("a").remove();h.removeClass("notification-with-button");h.removeClass("notification-for-collections")
}},replace:true}}]);
//
// source/angular/directives/lazyload.js
//
"use strict";angular.module("allrecipes").directive("lazyLoad",["$window",function(a){var c=700;var b=function(d){var i=d.getBoundingClientRect();
var e=a.innerHeight||a.document.documentElement.clientHeight;var j=a.innerWidth||a.document.documentElement.clientWidth;var h=i.top>=-c&&i.top<=e+c;
var f=i.bottom>=-c&&i.bottom<=e+c;var g=i.left>=0&&i.left<=j;return(h||f)&&g};return{restrict:"A",scope:{originalSrc:"@",src:"@"},link:function(d,f){var g=f[0];
var h=function(){if(!b(g)){return}var l=angular.element(g);var k=d.originalSrc;var i=d.src;if(k!==i){var j=f[0].style.display;
f[0].style.display="none";l.attr("src",k);f.unbind("load",h);angular.element(a).unbind("scroll",e);angular.element(a).unbind("resize",h);
f.parents().filter(function(){return $(this).css("overflow")=="scroll"}).unbind("scroll",h);f[0].style.display=j}};f.bind("load",h);
var e=_.debounce(h,50);angular.element(a).bind("scroll",e);angular.element(a).bind("resize",h);f.parents().filter(function(){return $(this).css("overflow")=="scroll"
}).bind("scroll",h);angular.element(document).ready(e)}}}]);
//
// source/angular/directives/recipemasonrygridview.js
//
"use strict";angular.module("allrecipes").directive("recipeMasonryGridView",function(){return{restrict:"A",templateUrl:function(b,a){return"/templates/"+a.type+"MasonryGridView.html"
},terminal:true,replace:false}});
//
// source/angular/directives/recipegridcard.js
//
"use strict";angular.module("allrecipes").directive("recipeGridCard",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/recipeGridCard.html"}
}]);
//
// source/angular/directives/publicProfileRightRail.js
//
"use strict";angular.module("allrecipes").directive("publicProfileRightRail",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/profile/public/publicProfileRightRail.html"}
}]);
//
// source/angular/directives/recipeGridCardSubmitterDetails.js
//
"use strict";angular.module("allrecipes").directive("recipeGridCardSubmitterDetails",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/RecipeGridCardSubmitterDetails.html"}
}]);
//
// source/angular/directives/reviewGridCard.js
//
"use strict";angular.module("allrecipes").directive("reviewGridCard",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/reviewGridCard.html"}
}]);
//
// source/angular/directives/collectionGridCard.js
//
"use strict";angular.module("allrecipes").directive("collectionGridCard",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/CollectionGridCard.html"}
}]);
//
// source/angular/directives/cookGridCard.js
//
"use strict";angular.module("allrecipes").directive("cookGridCard",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/CookGridCard.html"}
}]);
//
// source/angular/directives/photoGridCard.js
//
"use strict";angular.module("allrecipes").directive("photoGridCard",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/PhotoGridCard.html"}
}]);
//
...
}var c=b.href;if(b.internalReferrerLink==""){return c}if(b.href.indexOf("?")<0){d+="?"}else{d+="&"}d+="internalSource="+b.internalReferrerLink;
...
}}if(f.linkName){d+="&linkName="+f.linkName}if(f.clickId){d+="&clickId="+f.clickId}var g=c+d;return g}}]);angular.module("allrecipes").directive("internalReferrerLink",["internalReferrerLinker",function(a){return{restrict:"A",scope:{linkName:"@",clickId:"@"},priority:1,link:function(d,c,b){var e=a(d,c,b);
b.$set("href",e)}}}]);
//
...
//
angular.module("allrecipes").directive("cookRecommendationCard",function(){return{restrict:"E",replace:true,templateUrl:"/templates/CookRecommendation.html"}
});
//
// source/angular/directives/starRating.js
//
"use strict";angular.module("allrecipes").directive("starRating",function(){return{restrict:"A",scope:{rating:"@",reviewCount:"@"},templateUrl:"/templates/starrating.html",link:function(h,c,a){var g,e,f,d,i;
a.$observe("rating",function(k){g=5;e="http://images.media-allrecipes.com/ar-images/icons/rating-stars/full-star.png";f="http://images.media-allrecipes.com/ar-images/icons/rating-stars/half-star.png";
d="http://images.media-allrecipes.com/ar-images/icons/rating-stars/empty-star.png";i=b(k);h.stars=[];for(var j=0;j<5;j++){h.stars.push({src:i[j]})
}});function b(p){var r=[];var q=(Math.round(p*2)/2).toFixed(1);var o=Math.floor(q);for(var m=0;m<o;m++){r.push(e)}if(q!=o){r.push(f)
}var l=r.length;for(var n=0;n<g-l;n++){r.push(d)}return r}}}});
//
// source/angular/controllers/deferredActionCtrl.js
//
"use strict";angular.module("allrecipes").controller("ar_controllers_deferredAction",["$scope","$window","$interval","$location","ar_services_shopping_list","arRecipeSave","ar_services_made_it","datalayerEvent","arFollowingSave","arRecipeBoxStorage","ngDialog","$anchorScroll","$rootScope",function(e,f,b,c,q,t,p,u,r,s,y,a,d){var v;
var w=0;var x=50;e.destroyInterval=function(){if(angular.isDefined(v)){b.cancel(v);v=undefined}};var C=function(H){var I=[];H.items.forEach(function(K){I.push(ar.models.shoppingListWriteInPostItem(K.item,K.aisle))
});var J=ar.models.shoppingListWriteInPost(I,H.deferredActionName);o(J)};var B=function(H){var I=ar.models.shoppingListWriteInPost([ar.models.shoppingListWriteInPostItem(H.item,H.aisle)],H.deferredActionName);
o(I)};var E=function(H){var I=[];H.recipes.forEach(function(K){I.push(ar.models.shoppingListSaveRecipePostItem(K.recipeid,K.servings))
});var J=ar.models.shoppingListSaveRecipePost(I,H.deferredActionName);m(J)};var G=function(H){var I=ar.models.shoppingListSaveRecipePost([ar.models.shoppingListSaveRecipePostItem(H.recipeid,H.servings)],H.deferredActionName);
m(I)};var D=function(H){var J=[];H.recipes.forEach(function(K){J.push(ar.models.recipeBoxSaveRecipePostItem(K.RecipeID,K.Type))
});var I=ar.models.recipeBoxSaveRecipePost(J,H.deferredActionName);k(I)};var F=function(H){var I=ar.models.recipeBoxSaveRecipePost([ar.models.recipeBoxSaveRecipePostItem(H.RecipeID,H.Type)],H.deferredActionName);
k(I)};e.wireupAdIntegrationListeners=function(){if(!f.pubsub.isListening("A.ShoppingList.Add.Event","DeferredAction.Controller")){f.pubsub.listen("A.ShoppingList.Add.Event","DeferredAction.Controller",B)
}if(!f.pubsub.isListening("A.ShoppingList.AddMultiple.Event","DeferredAction.Controller")){f.pubsub.listen("A.ShoppingList.AddMultiple.Event","DeferredAction.Controller",C)
}if(!f.pubsub.isListening("A.ShoppingList.AddRecipe.Event","DeferredAction.Controller")){f.pubsub.listen("A.ShoppingList.AddRecipe.Event","DeferredAction.Controller",G)
}if(!f.pubsub.isListening("A.ShoppingList.AddMultipleRecipe.Event","DeferredAction.Controller")){f.pubsub.listen("A.ShoppingList.AddMultipleRecipe.Event","DeferredAction.Controller",E)
}if(!f.pubsub.isListening("A.Recipe.Save.ClickEvent","DeferredAction.Controller")){f.pubsub.listen("A.Recipe.Save.ClickEvent","DeferredAction.Controller",n)
}if(!f.pubsub.isListening("A.RecipeBox.AddRecipe.Event","DeferredAction.Controller")){f.pubsub.listen("A.RecipeBox.AddRecipe.Event","DeferredAction.Controller",F)
}if(!f.pubsub.isListening("A.RecipeBox.AddMultipleRecipe.Event","DeferredAction.Controller")){f.pubsub.listen("A.RecipeBox.AddMultipleRecipe.Event","DeferredAction.Controller",D)
}if(!f.pubsub.isListening("A.Cook.Save.ClickEvent","DeferredAction.Controller")){f.pubsub.listen("A.Cook.Save.ClickEvent","DeferredAction.Controller",g)
}if(!f.pubsub.isListening("Recipe.OpenCollectionModal","DeferredAction.Controller")){f.pubsub.listen("Recipe.OpenCollectionModal","DeferredAction.Controller",z)
}if(!f.pubsub.isListening("A.RecipeDetial.Rate.ClickEvent","DeferredAction.Controller")){f.pubsub.listen("A.RecipeDetial.Rate.ClickEvent","DeferredAction.Controller",A)
}};e.executePostLoginEvents=function(){if(f.location.href.indexOf("/account")>=0){return}var L=f.location.search.split("&");if(L.length===0){return
}var I=L.filter(function(M){return M.indexOf("deferred")>=0});if(I.length===0){return}var J=I[0].split("=");if(J.length!==2){return
}var K=JSON.parse(decodeURIComponent(J[1].replace(/\+/g," ")));if(!K||!K.deferredActionName){return}var H=K.deferredActionName.toLowerCase();
switch(H){case"addtorecipebox":n(K,true);break;case"addtoshoppinglist":o(K);break;case"addingredienttoshoppinglist":h(K);break;
case"addrecipetoshoppinglist":l(K);break;case"addpersonalrecipetoshoppinglist":j(K);break;case"addmadeit":i();break;case"addfollowing":g(K);
break;case"addrecipetoshoppinglistfromad":m(K);break;case"addrecipetorecipeboxfromad":k(K);break;case"openratingsection":A();
break}};function A(){f.pubsub.broadcast("A.Recipe.RateAndReview.ClickEvent",[true,{fromStickyNav:true}]);c.url(c.path());c.hash("recipe-toolbar");
a()}function n(I,H){s.hasRecipeId(I.recipeID,I.type).then(function(J){if(J){f.pubsub.broadcast("Recipe.OpenCollectionModal",[{recipeId:I.recipeID,recipeType:I.type,recipeTitle:I.title,recipeImageUrl:I.imageUrl}])
}else{t.save(I.recipeID,I.type,I.title,I.imageUrl,I.source,{success:function(){d.$broadcast("updateHeart",I.recipeID);f.pubsub.broadcast("Recipe.AddToRecipeBox",[{recipeId:I.recipeID}]);
f.pubsub.broadcast("Recipe.OpenCollectionModal",[{recipeId:I.recipeID,recipeType:I.type,recipeTitle:I.title,recipeImageUrl:I.imageUrl}]);
u.push("add to recipe box",I.recipeID,"Action Complete",I.source);c.url(c.path())},failure:function(){if(H){e.$emit("notify","Oops! We couldn’t add item to the Recipe Box. Try again in just a moment.",null,"failure")
}}})}})}function o(H){var I=q.saveWriteIns(H);I.$promise.then(function(){e.$emit("notify","Shopping list saved.",null,"success")
},function(){e.$emit("notify","Oops! We couldn’t add item to the Shopping List. Try again in just a moment.",null,"failure")})
}function m(H){var I=q.saveRecipeFromAd(H);I.$promise.then(function(){e.$emit("notify","Recipe(s) saved to shopping list.",null,"success")
},function(){e.$emit("notify","Oops! We couldn’t save the recipe(s) to the Shopping List. Try again in just a moment.",null,"failure")
})}function z(I){if(f.Toggles.Collections===true){e.data=I;e.isPhoneViewPort=(f.innerWidth<768);var H={template:"/templates/RecipeSaveCollectionModal.html",className:"ngdialog-theme-default ngdialog-theme-collection",scope:e};
y.openConfirm(H)}else{e.$emit("notify","Recipe saved",null,"success")}}function k(H){s.filterSavedRecipeIds(H.recipes).then(function(I){H.recipes=I;
if(H.recipes.length==0){e.$emit("notify","Recipe(s) saved to recipe box.",null,"success")}else{t.saveRecipeFromAd(H,{success:function(){e.$emit("notify","Recipe(s) saved to recipe box",null,"success");
H.recipes.forEach(function(J){d.$broadcast("updateHeart",J.RecipeID);f.pubsub.broadcast("Recipe.AddToRecipeBox",[{recipeId:J.RecipeID}]);
u.push("add to recipe box",J.recipeId,"Action Complete",J.source)})},failure:function(){e.$emit("notify","Oops! We couldn’t add recipe(s) to the Recipe Box. Try again in just a moment.",null,"failure")
}})}})}function h(H){var I=q.saveIngredient(H);I.$promise.then(function(){v=b(function(){if(w>x){e.destroyInterval()}var J=angular.element('input[data-id="'+H.ingredientid+'"]');
if(J.length>0){J.prop("checked",true);e.destroyInterval()}w++},100);e.$emit("notify","Added to shopping list.",null,"success")
},function(){e.$emit("notify","Oops! We couldn’t add ingredient to the Shopping List. Try again in just a moment.",null,"failure")
})}function l(H){var I=q.saveRecipe(H);I.$promise.then(function(){e.$emit("notify","Shopping list saved.",null,"success")},function(){e.$emit("notify","Oops! We couldn’t add recipe to the Shopping List. Try again in just a moment.",null,"failure")
})}function j(H){var I=q.savePersonalRecipe(H);I.$promise.then(function(){e.$emit("notify","Shopping list saved.",null,"success")
},function(){e.$emit("notify","Oops! We couldn’t add recipe to the Shopping List. Try again in just a moment.",null,"failure")
})}function i(){f.pubsub.broadcast("A.MadeIt.Add.Event",[])}function g(H){r.save(H.userID,{success:function(){d.$broadcast("updateHeart",H.userID)
},failure:function(){}})}}]);
//
...
//
"use strict";angular.module("allrecipes").factory("ar_services_environment",["$window",function(a){return{url:(a.location.host=="m.perf01-vm.allrecipes.corp")?"https://apps.perf05-vm.allrecipes.corp/":a.location.host.replace(/^(m|next|usseamobile01|usseamobile02|usseamobile03|usseamobile04|usseamobile05|preview)\./i,"https://apps.")+"/",baseDomain:a.document===undefined||a.document.domain===undefined||a.document.domain.indexOf(".corp")>=0?".allrecipes.corp":".allrecipes.com",imageServerUrl:a.location.host.indexOf("allrecipes.com")>0?"http://images.media-allrecipes.com/":"http://images.preview.allrecipes.corp/"}
}]);
//
...
//
"use strict";angular.module("allrecipes").factory("perishableLocalstorage",["localStorageService",function(b){var c=1800000;var a=function(d){return b.get(d+"ExpirationDate")===null||Date.now()>b.get(d+"ExpirationDate")
};return{set:function(d,e){b.set(d,e);if(a(d)){b.set(d+"ExpirationDate",Date.now()+c)}},get:function(d){return(a(d)?null:b.get(d))
},remove:function(d){b.remove(d+"ExpirationDate");return b.remove(d)}}}]);
//
// source/angular/services/followingsave-service.js
//
"use strict";angular.module("allrecipes").factory("arFollowingSave",["$window","ar_services_following","arFollowingStorage","ar_current_user_state_service","ar_public_profile_state_service",function(a,d,e,b,c){return{save:function(g,f){var h=ar.models.followPost(g,"AddFollowing");
var i=function(l){e.add(g);var j=b.getCurrentUser();if(j!==null&&j!==undefined){j.FollowingCount++;b.setCurrentUser(j);b.followingListChangedHandler.Fire();
var k=c.getProfileUser();if(k!==null&&k!==undefined){k.FollowersCount++;if(j.Id===k.Id){k.FollowingCount++}c.setProfileUser(k)
}a.pubsub.broadcast("FollowingEvent")}c.followingListChangedHandler.Fire();f.success(l)};d.save(h).$promise.then(i,f.failure)
},remove:function(g,f){var h=function(k){e.remove(g);var i=b.getCurrentUser();if(i!==null&&i!==undefined){i.FollowingCount--;
b.setCurrentUser(i);b.followingListChangedHandler.Fire();var j=c.getProfileUser();if(j!==null&&j!==undefined){j.FollowersCount--;
if(i.Id===j.Id){j.FollowingCount--}c.setProfileUser(j)}a.pubsub.broadcast("FollowingEvent")}c.followingListChangedHandler.Fire();
f.success(k)};d.remove({id:g}).$promise.then(h,f.failure)}}}]);
//
...
//
"use strict";angular.module("allrecipes").factory("arRecipeSave",["$window","ar_services_recipebox","arRecipeBoxStorage","ar_current_user_state_service","ar_public_profile_state_service",function(a,d,e,b,c){return{save:function(j,h,m,g,k,f){var i=ar.models.recipeboxPost(j,"recipe",m,g,"AddToRecipeBox",k);
...
//
"use strict";angular.module("allrecipes").factory("datalayerEvent",["$window","$location",function(b,a){return{push:function(i,h,c,d,f,m,k,l,j){if(k==null){k={}
...
b.dataLayer.event.push(e);b.pubsub.broadcast("An.Event.ToBe.Tracked",[g,e.eventInfo.eventName,parseInt(a.search().page)])},pushError:function(h,g,c,d){var f=UID.New();
...
switch(g){case"recipe":case"personalrecipe":e=(g=="recipe")?"recipe":"personal-recipe";j=d(f.title);h=f.recipeID;break;case"review":e=(f.recipe.itemType.toLowerCase()=="recipe")?"recipe":"personal-recipe";
j=d(f.recipe.title);h=f.recipe.recipeID;break}i+="/"+e+"/"+h+"/"+j+"/";if(f.recipeDetailUrlSuffix){if(g!="review"){i+=f.recipeDetailUrlSuffix
}}}return i},getSearchUrl:function(g,f,e,j){var h=c(g,f,e);var k="&sort="+(j?j:"re");var i="/search/results?"+h+k;return i},getQueryStringValue:function(h){var g=a.location.search.substring(1);
...
//
angular.module("allrecipes").factory("feedProvider",["ar_services_environment","ar_services_token","$http",function(c,d,a){var e={};
var b=function(f){return !!e[f]};return{get:function(g){angular.extend({page:1,includeCookRecs:false,includeCookActivities:false},g);
if(b(g.page)){return{then:function(){}}}e[g.page]=true;var f=a.get(c.url+"v1/user-feed?page="+g.page+"&includeCookRecs="+g.includeCookRecs+"&includeCookActivities="+g.includeCookActivities,{headers:{Authorization:d.token}});
return f}}}]);
//
...
//
"use strict";angular.module("allrecipes").controller("ar_controller_collections",["$scope","$window","ngDialog","ar_services_collections","datalayerEvent",function(a,b,f,c,d){a.errorMessage="";
a.recipeDetail={};a.collectionList={};a.modalOpenLocation=null;a.init=function(h){a.recipeDetail=h;a.collectionList=e()};a.openCollectionModal=function(i){a.modalOpenLocation=i;
var h={template:"/templates/CollectionCreateModal.html",className:"ngdialog-theme-default ngdialog-theme-collection",scope:a};
f.openConfirm(h)};a.collectionOptionSelected=function(h){h.selected=!h.selected};a.createAndAddNewCollection=function(i){var h={collectionName:i,description:""};
a.submitCollection(h,true,false);a.form.collectionName=""};a.submitCollection=function(h,j,i){if(j){a.saved=c.save({collectionId:0,name:h.collectionName,description:h.description});
...
}d.push(StringHelper.IsNullOrEmpty(h.description)?"create collection":"create collection with description",l.collectionId,"Action Complete",k);
...
}})}};var g=function(i,h){var j={folderID:h,name:i,selected:true};a.collectionList.unshift(j)};var e=function(){a.saved=c.getCollection();
a.saved.$promise.then(function(h){angular.forEach(h.collectionList,function(i){i.selected=false});a.collectionList=h.collectionList
},function(h){if(h.status!==401){a.errorMessage=h.data.message}})};a.submitCollectionList=function(){var h=[];angular.forEach(a.collectionList,function(j){if(j.selected===true){var i={folderID:null,name:null};
i.folderID=j.folderID;i.name=j.name;h.push(i);d.push("add recipe to collection",j.folderID,"Action Complete","add recipe to collection from recipe")
}});if(h.length>0){a.update=c.addRecipeToCollectionList({recipeId:a.recipeDetail.recipeId,recipeType:a.recipeDetail.recipeType,collectionList:h});
a.update.$promise.then(function(){a.$emit("notify","Added to ",5000,"success",null,null,null,h[0].name,h[0].folderID,h.length-1);
f.close()},function(i){if(i.status!==401){a.errorMessage=i.data.message}})}else{f.close()}};a.submitEditCollection=function(h,i,j){if(j){a.update=c.save({collectionId:h.collectionId,name:h.collectionName,description:h.collectionDescription});
a.update.$promise.then(function(){if(!h.breadcrumbName){h.breadcrumbName=""}i.collectionName=h.breadcrumbName+h.collectionName;
i.collectionDescription=h.collectionDescription;f.close()},function(k){if(k.status!==401){a.errorMessage=k.data.message}})}}}]);
//
// source/angular/controllers/collectionDetailsCtrl.js
//
"use strict";angular.module("allrecipes").controller("ar_collection_details_controller",["$scope","$window","$timeout","ar_services_collections","ar_services_recipebox","ar_current_user_state_service","ngDialog","ar_services_shopping_list","datalayerEvent",function(a,c,b,f,g,e,i,d,h){a.user={};
...
a.itemList=[];a.sortType="dateAdded";a.bandItemList=[];a.allIdsInCollection=[];a.collectionOwner=null;a.collectionsTools="collapsed";
a.currentPage=1;a.currentBandPage=1;a.pageNumber=1;a.pageSize=20;a.collection={collectionName:null,collectionDescription:null,collectionId:0};
a.editFormCollection={collectionName:null,collectionDescription:null,collectionId:0};a.init=function(j){e.currentUserChangedHandler.Add(function(k){a.user=k
});a.user=e.getCurrentUser();a.collection.collectionId=j;a.getAllDetails(j);if(a.user!==null&&a.user!==undefined){a.getRecipesForUser(a.user.Id,a.currentBandPage)
}};a.handleScroll=function(j){if(j.target.id==="recipe-scroll-band"&&j.target.scrollLeft===(j.target.scrollWidth-j.target.offsetWidth)){a.getNextBandPage()
}};a.handleRemoveItem=function(k){var j=a.allIdsInCollection.indexOf(parseInt(k,10));if(j>=0){a.allIdsInCollection.splice(j,1)
}a.enableBandItem(k);a.removeItemFromCollection(k,a.collection.collectionId)};a.getNextPage=function(){a.getCollectionRecipes(a.collection.collectionId)
};a.getNextBandPage=function(){a.getRecipesForUser(a.user.Id,a.currentBandPage+1)};a.addToCollection=function(j,l,k){a.allIdsInCollection.push(j);
a.disableBandItem(j);a.addRecipeToCollection(l,j,a.collection.collectionId,k)};a.updateBandRecipesIfInCollection=function(k,j){if(j===null||j===undefined||j.length===0||k===null||k===undefined||k.length===0){return
}angular.forEach(k,function(l){l.isInCollection=j.indexOf(l.savedItemId)!==-1})};a.disableBandItem=function(k){var j=0;for(;j<a.bandItemList.length;
j+=1){if(a.bandItemList[j].savedItemId===parseInt(k)){a.bandItemList[j].isInCollection=true}}};a.enableBandItem=function(k){var j=0;
for(;j<a.bandItemList.length;j+=1){if(a.bandItemList[j].savedItemId===parseInt(k)){a.bandItemList[j].isInCollection=false}}};
a.openEditCollectionModal=function(){a.collectionsTools="collapsed";var j={template:"/templates/CollectionEditModal.html",className:"ngdialog-theme-default ngdialog-theme-collection",scope:a};
i.openConfirm(j)};a.openDeleteModal=function(){var j={template:"/templates/CollectionDeleteModal.html",className:"ngdialog-theme-default ngdialog-theme-collection-delete",scope:a};
i.openConfirm(j);return j};a.toggleCollectionsTools=function(){if(a.collectionsTools==="collapsed"){a.collectionsTools="exposed"
}else{a.collectionsTools="collapsed"}};a.handleNewBandRecipes=function(l){var k=l.recipes.map(function(m){m.recipeSummary.savedItemId=m.savedItemId;
return m.recipeSummary});var j=a.bandItemList.concat(k);a.bandItemList=j;a.currentBandPage=l.metaData.page;a.updateBandRecipesIfInCollection(a.bandItemList,a.allIdsInCollection);
a.hasBandLoaded=true};a.handleNewDetails=function(k){a.editFormCollection.collectionId=k.id;a.editFormCollection.collectionName=k.name;
a.editFormCollection.collectionDescription=k.description;a.editFormCollection.breadcrumbName=k.breadcrumbName;var j="";if(k.breadcrumbName){j=k.breadcrumbName
}a.collection.collectionId=k.id;a.collection.collectionName=j+k.name;a.collection.collectionDescription=k.description;a.collection.breadcrumbName=k.breadcrumbName;
a.collectionOwner=k.owner;a.isCollectionOwner=a.checkCollectionOwner();a.allIdsInCollection=k.allIds;a.handleNewRecipes(k.recipes);
a.imageUrl=k.owner.photo.urls[0].url;a.updateBandRecipesIfInCollection(a.bandItemList,a.allIdsInCollection);a.hasLoaded=true};
a.handleRecipeAdded=function(l,k){h.push("add recipe to collection",a.collection.collectionId,"Action Complete","add recipe to collection from collection page");
var j=0;for(;j<a.bandItemList.length;j+=1){if(a.bandItemList[j].savedItemId===parseInt(k)){a.bandItemList[j].subType="remove";
a.itemList.unshift(a.bandItemList[j]);break}}a.totalItemsInCollection+=1;b(function(){a.$emit("ReloadMasonry")},250).then(function(){window.scroll(0,1);
window.scroll(0,0)})};a.handleRecipeAddFailed=function(l,k){a.enableBandItem(k);var j=a.allIdsInCollection.indexOf(parseInt(k,10));
if(j>=0){a.allIdsInCollection.splice(j,1)}};a.handleRecipeRemoved=function(k){h.push("remove recipe from collection",a.collection.collectionId,"Action Complete","remove recipe from collection");
var j=0;for(;j<a.itemList.length;j+=1){if(a.itemList[j].savedItemId===parseInt(k)){break}}if(j<a.itemList.length){a.itemList.splice(j,1)
}a.totalItemsInCollection-=1};a.handleRecipeRemoveFailed=function(j){a.disableBandItem(j);a.allIdsInCollection.push(j)};a.handleNewRecipes=function(k){a.currentPage=k.metaData.page;
a.pageNumber=k.metaData.page+1;var j=k.recipes.map(function(l){l.recipeSummary.subType=a.isCollectionOwner===true?"remove":"share";
l.recipeSummary.savedItemId=l.savedItemId;return l.recipeSummary});Array.prototype.push.apply(a.itemList,j);a.showMore=k.metaData.totalCount>(k.metaData.page*k.metaData.pagesize);
a.totalItemsInCollection=k.metaData.totalCount};a.handleCollectionDeleted=function(){h.push("delete collection",a.collection.collectionId,"Action Complete","delete collection");
...
}return false};a.getCollectionRecipes=function(j){f.getDetails({collectionId:j,page:a.pageNumber,start:a.itemList.length+1,pagesize:a.pageSize}).$promise.then(function(k){a.handleNewRecipes(k.recipes)
})["finally"](function(){})};a.getAllDetails=function(j){f.getDetails({collectionId:j,page:a.pageNumber,start:a.itemList.length+1,pagesize:a.pageSize}).$promise.then(function(k){a.handleNewDetails(k)
})["finally"](function(){})};a.deleteCollection=function(j){f.del({collectionId:j}).$promise.then(function(k){a.handleCollectionDeleted(k)
})["finally"](function(){})};a.getRecipesForUser=function(k,j){g.getRecipeBoxRecipes({page:j,userId:k,sort:a.sortType}).$promise.then(function(l){a.handleNewBandRecipes(l)
})["finally"](function(){})};a.addRecipeToCollection=function(l,k,j,m){a.added=f.addRecipeToCollection({collectionId:j,itemId:l,type:m});
a.added.$promise.then(function(){a.handleRecipeAdded(l,k)},function(){a.handleRecipeAddFailed(l,k)})};a.removeItemFromCollection=function(k,j){a.removed=f.removeRecipeFromCollection({collectionId:j,itemId:k});
a.removed.$promise.then(function(){a.handleRecipeRemoved(k)},function(){a.handleRecipeRemoveFailed(k)})};a.addRecipesToShoppingList=function(){pubsub.broadcast("ShoppingListSave");
a.result=d.saveCollection({collectionId:a.collection.collectionId});a.result.$promise.then(function(){a.$emit("notify","Added!",null,"success")
...
//
"use strict";angular.module("allrecipes").directive("collectionDetails",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/CollectionDetails.html"}
}]);
//
// source/angular/directives/recipeBand.js
//
"use strict";angular.module("allrecipes").directive("recipeBand",[function(){return{restrict:"E",replace:true,templateUrl:"/templates/RecipeBand.html"}
}]);
//
...
//
"use strict";angular.module("allrecipes").factory("arSaveItemLink",["arRecipeSave","$rootScope","arRecipeBoxStorage","arLogin","datalayerEvent","arFollowingSave","arFollowingStorage","ar_public_profile_state_service","$window",function(h,a,g,f,i,d,e,c,b){return function(v,l,k){var y=(v.type)?v.type.toLowerCase():"";
var t=(y=="recipe"||y=="recipes"||y=="personalrecipe"||y=="personal-recipes"||y=="personal"||y=="custom");var s=(y=="personalrecipe"||y=="personal-recipes"||y=="personal");
var q=(!t&&y=="cook");var u="";var r=f.isLoggedIn();if(t){g.hasRecipeId(v.id,v.type).then(function(z){v.saved=z;v.hoverText=(v.saved?"Remove this recipe from your favorites":"Save this recipe");
v.buttonText=(v.saved?"Remove from Favorites":"Save to Favorites")})}else{if(q){e.hasCookId(v.id).then(function(z){v.saved=z;
v.hoverText=(v.saved?"Stop following this cook":"Follow this cook")})}else{if(v.updateHeart){v.saved=true;v.hoverText=(v.type=="cook"?"Stop following this cook":"Remove this recipe from your favorites");
if(v.type=="Recipe"){v.buttonText="Remove from Favorites"}}}}var w=function(z){u=z.recipeBoxItemID;if(v.saved){b.pubsub.broadcast("Recipe.AddToRecipeBox",[{recipeId:v.id}]);
b.pubsub.broadcast("Recipe.OpenCollectionModal",[{recipeId:v.id,recipeType:v.type,recipeTitle:v.name,recipeImageUrl:v.imageurl}]);
i.push("add to recipe box",v.id,"Action Complete","recipe button")}else{if(b.Toggles.Collections!==true){v.$emit("notify","Recipe "+(v.saved?"saved":"removed"),null,"success")
}else{v.$emit("notify","Recipe removed",null,"success")}}v.hoverText=(v.saved?"Remove this recipe from your favorites":"Save this recipe");
a.$broadcast("favoriteChanged",v.saved);v.buttonText=(v.saved?"Remove from Favorites":"Save to Favorites")};var m=function(z){u=0
};var x=function(){if(v.saved){a.$broadcast("cookFollowRemove",v.cookId);var z=v.clickid||"cook card";var B="follow cook";var A="Action Complete";
if(k.analyticsType){i.push(B,v.id,A,z,null,null,{internalSource:k.analyticsType})}}else{a.$broadcast("cookFollowAdd",v.cookId)
}v.hoverText=(v.saved?"Stop following this cook":"Follow this cook")};var n=function(){};var o=function(){if(v.saved){if(v.recipeboxitemid&&v.recipeboxitemid>0){h.removeFavorite(v.id,v.recipeboxitemid,v.type,{success:w,failure:m})
}else{if(!s){h.remove(v.id,v.type,{success:w,failure:m})}else{m(null)}}}else{h.save(v.id,v.type,null,null,"recipe button",{success:w,failure:m})
}v.saved=!v.saved};var p=function(){if(v.saved){d.remove(v.id,{success:x,failure:n})}else{d.save(v.id,{success:x,failure:n})}v.saved=!v.saved
};var j=function(){if(t){o()}else{if(q){p()}}};l.off().on("click",function(z){var A=(z.target.tagName=="SPAN"&&z.target.getAttribute("name")=="AddRemovefavorites");
if(!A){if(!r){if(q){b.pubsub.broadcast("A.Cook.Save.ClickEvent",[{userID:v.id}])}else{if(t){b.pubsub.broadcast("A.Recipe.Save.ClickEvent",[{recipeID:v.id,type:y,title:v.name,imageUrl:v.imageurl,source:"recipe button"}])
}}}f.ensureUserIsLoggedIn(function(){j()})(f.isLoggedIn())}})}}]);angular.module("allrecipes").directive("arSaveRecipeButton",["arSaveItemLink",function(a){return{restrict:"E",template:'<span ng-bind="buttonText" name="AddRemovefavorites"></span>',scope:{id:"=?",type:"=?",buttonText:"=?",},controller:["$scope","$element",function(c,b){c.$on("favoriteChanged",function(){c.updateHeart=true;
a(c,b)});c.$on("updateHeart",function(e,d){if(c.id==d){c.updateHeart=true;a(c,b)}})}],replace:true,link:a}}]);angular.module("allrecipes").directive("arSaveItem",["arSaveItemLink",function(a){return{restrict:"E",template:"<a class='favorite' ng-class='{ highlighted : saved }' title='{{ hoverText }}'></a>",scope:{id:"=?",type:"=?",recipeboxitemid:"=?",clickid:"=?",name:"=?",imageurl:"=?",},controller:["$scope","$element",function(c,b){c.$on("cookChange",function(e,d){console.log(e);
console.log(d);c.id=d;c.type="cook";c.clickid="header";c.recipeboxitemid="";c.name="";c.imageurl="";a(c,b)});c.$on("updateHeart",function(e,d){if(c.id==d){c.updateHeart=true;
a(c,b)}})}],replace:true,link:a}}]);angular.module("allrecipes").directive("arSaveCook",["arSaveItemLink",function(a){return{restrict:"E",template:"<a ng-class='{ highlighted : saved }' title='{{ hoverText }}'><span></span></a>",scope:{id:"=?",type:"=?",clickid:"=?"},controller:["$scope","$element",function(c,b){c.$on("cookChange",function(e,d){console.log(e);
console.log(d);c.id=d;c.type="cook";c.clickid="header";a(c,b)});c.$on("updateHeart",function(e,d){if(c.id==d){c.updateHeart=true;
a(c,b)}})}],replace:true,link:a}}]);
//
...
}}d=j;b.$apply(function(){a.search("page",d).replace()});if(i){e.push(i,null,"Action Complete")}};var h=function(){for(var j=f.length-1;
j>=0;j--){if(c.pageYOffset>=f[j].top()){var k=f[j].page;if(d!==k){g(k)}return}}if(f[0]&&d!==f[0].page){g(f[0].page)}};angular.element(c).on("scroll",h);
return{registerPageAnchor:function(i){f.push(i);f.sort(function(j,k){return j.page-k.page})}}}]);angular.module("allrecipes").directive("a",function(){return{restrict:"E",link:function(c,b,a){if((a.href||a.ngHref)&&!a.target&&!b.data("spa")){b[0].target="_self"
...
},100)}}}}]);angular.module("allrecipes").directive("arInfiniteScroll",["$window","$location",function(b,a){return{restrict:"A",link:function(f,e){var d=1000;
...
//
"use strict";angular.module("allrecipes").directive("formatLargeNumber",["largeNumberDisplayFilter",function(a){return{restrict:"E",link:function(d,c,b){c.replaceWith(a(b.number));
return}}}]);
//
// source/angular/directives/recipeLink.js
//
"use strict";angular.module("allrecipes").directive("recipeLink",["ar_services_url","internalReferrerLinker",function(b,a){return{restrict:"A",scope:true,replace:false,transclude:true,link:function(f,d,c){if(f.item&&f.item.itemType){var e=f.item.itemType.toLowerCase();
f.recipeDetailUrl=b.getRecipeUrl(f.item);if(e=="review"){f.recipeDetailEditReviewUrl=f.recipeDetailUrl+"?editReview=1"}if(c.internalReferrer){c.href=f.recipeDetailUrl;
...
//
angular.module("allrecipes").directive("scrollEvent",function(){return function(c,b,a){b.on("scroll",function(d){if(typeof c.handleScroll==="function"){c.handleScroll(d)
}})}});
//
// source/angular/directives/globalClickEvent.js
//
angular.module("allrecipes").directive("globalClickEvent",["$document",function(a){return{restrict:"A",link:function(e,d,b,c){if(b.ignoreSelfClick!==undefined&&b.ignoreSelfClick!==null){d.bind("click",function(f){f.stopPropagation()
})}a.bind("click",function(){e.$apply(b.globalClickEvent)})}}}]);
//
...
//
"use strict";angular.module("allrecipes").filter("entityHtml",["$sce",function(a){return function(b){return a.trustAsHtml(b)}
}]);
//
...
//
"use strict";angular.module("allrecipes").controller("ar_controllers_top_nav",["$scope","$rootScope",function(b,a){var c="none";
var e=false;var f=false;var g=false;var h=false;var d=function(j){switch(j){case 1:return e;case 2:return f;case 3:return g;case 4:return h
}return false};var i=function(j){switch(j){case 1:e=!e;f=false;g=false;h=false;break;case 2:f=!f;e=false;g=false;h=false;break;
case 3:g=!g;e=false;f=false;h=false;break;case 4:h=!h;e=false;f=false;g=false;break}};b.isTabActiveAndOpen=function(j){return c===j&&d(j)
};b.toggleActiveTab=function(j){c=j;i(j)};a.$on("onEditSearchClick",function(){b.toggleActiveTab(1)})}]);
//
...
//
"use strict";angular.module("allrecipes").factory("ar_services_following",["ar_services_environment","$resource","ar_services_token",function(b,a,c){var d=b.url+"v1/users/me/following";
return a(d,{id:"@id"},{save:{url:d,method:"POST",isArray:false,headers:{Authorization:c.token}},remove:{url:d+"/:id",method:"DELETE",isArray:false,headers:{Authorization:c.token}},getAllFollowing:{url:d,method:"GET",isArray:false,headers:{Authorization:c.token}},getPublicFollowing:{url:b.url+"v1/users/:id/following",method:"GET",isArray:false,headers:{Authorization:c.token}},getAllFollowingIds:{url:d+"/userids",method:"GET",isArray:true,headers:{Authorization:c.token}}})
}]);
//
...
//
"use strict";angular.module("allrecipes").controller("ar_controllers_profile_photo",["$scope","$rootScope","$window","$location","ar_services_profile_photos","adManager","ar_services_url",function(c,b,d,a,f,e,k){c.showSortFiltersStyle="collapsed";
c.indicatorStyle="icon--chevron-down";c.toggleDisplayText="Sort";c.showEditTextArea=false;c.showDescription=true;if(a.search().edit){c.showEditTextArea=true;
c.showDescription=false}var h;var i=function(l){c.title=l.title;c.description=l.description;c.originalDescription=l.description;
c.imageUrl=l.urls[l.urls.length-1].url;c.cookId=l.submitter.userID;c.cookDisplayName=l.submitter.name;c.cookImageURL=l.submitter.photo.urls[0].url;
c.cookIsPro=l.submitter.isPro;c.followersCount=l.submitter.followersCount;c.recipeId=l.recipeId;c.recipeTitle=l.recipeTitle;c.recipeDetailUrl=(c.recipeId&&c.recipeId>0)?k.getRecipeUrl({itemType:"recipe",title:c.recipeTitle,recipeID:c.recipeId}):null;
c.cookProfile="/cook/"+c.cookId};c.init=function(){var l=d.PhotoGalleryInitScope;h=l.photoId;i(l);c.photoDictionary=l.photoCollection
};c.loadPhoto=function(l){h=l;f.getPhoto({photoId:l}).$promise.then(function(m){i(m);b.$broadcast("cookChange",c.cookId);if(d.karmaEnabled&&typeof d.refreshAdFrame==="function"){d.refreshAdFrame()
}else{e.RefreshAd("div-gpt-ad-photodetail-1")}});a.path("/photos/"+h+"/")};c.deletePhoto=function(){if(confirm(" Are you sure you want to delete this image?")){f.deletePhoto({photoId:h}).$promise.then(function(l){c.model=l;
c.$emit("notify","Photo deleted",null,"success",null,null,function(){d.location.href="/cook/my/photos/"})})}};c.updateDesc=function(){f.updatePhoto({photoId:h,description:c.description,title:c.title}).$promise.then(function(l){c.model=l;
c.$emit("notify","Description updated",null,"success");c.enableEdit();c.loadPhoto(h)})};c.toggleShowMenuOptions=function(){if(c.showSortFiltersStyle=="collapsed"){j()
}else{g()}};c.next=function(){return c.photoDictionary[h]==undefined?-1:c.photoDictionary[h].NextPhoto};c.prev=function(){return c.photoDictionary[h]==undefined?-1:c.photoDictionary[h].PrevPhoto
};c.getCookId=function(){return c.cookId};var j=function(){c.filterDisplayStatus="visible";c.toggleDisplayText="Hide";c.showSortFiltersStyle="exposed";
c.indicatorStyle="icon--chevron-up"};var g=function(){c.filterDisplayStatus="hidden";c.toggleDisplayText="Sort";c.showSortFiltersStyle="collapsed";
...
c.enableEdit()};c.makeProfilePhoto=function(){c.$emit("notify","Profile photo updated",null,"success",null,null,function(){d.location.href="/account/updateprofilephoto/?photoid="+h
})};c.$on("cookFollowRemove",function(){c.followersCount++});c.$on("cookFollowAdd",function(){c.followersCount--})}]);
//
// source/angular/controllers/homeCtrl.js
//
angular.module("allrecipes").controller("arControllersHome",["$scope","feedProvider","ads","feedItemConverter","$rootScope","$location",function(c,f,d,e,b,a){c.infiniteCounter=1;
c.infiniteScrollLimit=5;c.hasMoreRecipes=true;c.itemList=[];c.viewportOffset=1200;c.page=parseInt(a.search().page);if(isNaN(c.page)){c.page=1
}var g=function(){var i=parseInt(a.search().page);var h=isNaN(i)?c.page:i;return h};c.getMoreResults=function(){if(b.Toggles.Feed){f.get({page:g()+1,includeCookRecs:b.Toggles.CookRec,includeCookActivities:b.Toggles.Activity}).then(function(i){c.infiniteCounter++;
c.page=g()+1;var h=d.arrayWithAdsInserted(e(i.data.feed));if(h.length>0){h[0].pageMarker=c.page}if(h.length<20){c.hasMoreRecipes=false
}if(d.isRecipeCardSponsorLogoOn()&&h.length>0){d.loadNewSponsoredRecipes(h)}c.itemList=c.itemList.concat(h)})}}}]);
//
...
};var e=(d())?"/cook/"+f.item.userID+"/":"";f.publicProfileUrl=e;if(b.internalReferrer){b.href=e;b.internalReferrerLink=b.internalReferrer;
f.publicProfileUrl=a(f,c,b)}}}}]);
//
// source/angular/services/current-user-state-service.js
//
"use strict";angular.module("allrecipes").service("ar_current_user_state_service",["$window",function(b){var a=undefined;this.currentUserChangedHandler=new EventHandler("CurrentUserState.CurrentUserChanged");
this.followersListChangedHandler=new EventHandler("CurrentUserState.FollowersListChanged");this.followingListChangedHandler=new EventHandler("CurrentUserState.FollowingListChanged");
this.getCurrentUser=function(){if(a===undefined||a===null){var c=b.localStorage.getItem("CurrentUserStateModel");if(c!==""&&c!==undefined&&c!==null){a=JSON.parse(c)
}else{a=null}}return a};this.setCurrentUser=function(c){this.currentUserState=c;this.currentUserChangedHandler.Handle(this.currentUserState)
}}]);
//
// source/angular/services/public-profile-state-service.js
//
"use strict";angular.module("allrecipes").service("ar_public_profile_state_service",["$window",function(c){var a=undefined;var b=undefined;
this.publicProfileUserChangedHandler=new EventHandler("PublicProfile.PublicProfileUserChanged");this.publicProfileSubViewChangedHandler=new EventHandler("PublicProfile.PublicProfileSubViewChanged");
...
this.getProfileUser=function(){if(a===undefined||a===null){a=c.localStorage.getItem("PublicProfileStateModel")===null||c.localStorage.getItem("PublicProfileStateModel")===undefined||c.localStorage.getItem("PublicProfileStateModel")===""?null:JSON.parse(c.localStorage.getItem("PublicProfileStateModel"))
}return a};this.setProfileUser=function(d){this.profileUserState=d;this.publicProfileUserChangedHandler.Handle(this.profileUserState)
};this.getPublicProfileSubView=function(){return b};this.setPublicProfileSubView=function(e){var d=b;b=e;this.publicProfileSubViewChangedHandler.Handle({currentView:b,previousView:d})
}}]);
//
...
return a(d,{collectionId:"@id"},{getDetails:{url:d+"v1/collections/:collectionId",method:"GET",isArray:false,headers:{Authorization:c.token}},getCollection:{url:d+"v1/users/me/collection-summary",method:"GET",isArray:false,headers:{Authorization:c.token}},save:{url:d+"v1/collections",method:"POST",isArray:false,headers:{Authorization:function(){var e=c.token();
return e}}},addRecipeToCollection:{url:d+"v1/collections/additem",method:"POST",isArray:false,headers:{Authorization:function(){var e=c.token();
return e}}},addRecipeToCollectionList:{url:d+"v1/users/me/add-collection-list",method:"POST",isArray:false,headers:{Authorization:function(){var e=c.token();
return e}}},removeRecipeFromCollection:{url:d+"v1/collections/removeitem",method:"POST",isArray:false,headers:{Authorization:function(){var e=c.token();
return e}}},del:{url:d+"v1/collections/:collectionId",method:"DELETE",isArray:false,headers:{Authorization:function(){var e=c.token();
...
//
angular.module("allrecipes").factory("ar_services_file_reader",["$q","$window",function(a,b){var d=function(g,f,h){return function(){h.$apply(function(){f.resolve(g.result)
})}};var c=function(f,h){var g=new b.FileReader();g.onload=d(g,f,h);return g};var e=function(g,i){var f=a.defer();var h=c(f,i);
h.readAsDataURL(g);return f.promise};return{readAsDataUrl:e}}]);
//
...
a.select=function(f){angular.forEach(e,function(g){g.selected=false});f.selected=true;b(function(){c.pubsub.broadcast("MasonryGridReload")
...
};this.addPane=function(f){if(e.length==0){a.select(f)}e.push(f)}}]).directive("tabs",[function(){return{restrict:"A",transclude:true,scope:{fourTabs:"@"},controller:"tabsCtrl",template:'<div class="tabbable"><ul class="nav nav-tabs"><li ng-repeat="pane in panes" ng-class="{\'active\':pane.selected}"><a href="" ng-click="biTracking(pane.title); select(pane);" ng-bind="pane.title"></a></li></ul><div class="tab-content" ng-transclude></div></div>',replace:true}
}]).directive("pane",[function(){return{require:"^tabs",restrict:"A",scope:{title:"@"},link:function(c,b,a,d){d.addPane(c)},template:'<div class="tab-pane" ng-class="{active: selected}" class="active" ng-transclude></div>',replace:true,transclude:true,}
}]);
//
// source/angular/controllers/photoUploadCtrl.js
//
"use strict";angular.module("allrecipes").controller("ar_controllers_photo_upload",["$scope","$window","ar_services_photoupload","datalayerEvent","arLogin","$rootScope","$location","ngDialog","ar_services_file_reader","$document",function(d,e,g,j,h,c,b,p,f,a){d.rateRecipeButtonStyle="rateRecipe btns-two-small";
d.rateButtonIsHighlighted=false;d.enablePhotoUpload=h.ensureUserIsLoggedIn();d.hasUploadedPhoto=false;d.photoId=0;d.photoUrl=null;
d.isProfilePhoto=false;d.hasReviewedRecipe=false;d.init=function(v,t,s,u){d.userId=v||h.userId;d.recipeId=t||0;d.rateButtonIsHighlighted=d.hasReviewedRecipe=(s)?true:false;
d.uploadFrom=u||"recipe page";r();d.isProfilePhoto=(location.pathname.search("profile-settings")>0)?true:false};d.makeProfilePhoto=function(){d.isProfilePhoto=!d.isProfilePhoto
};var r=function(){e.pubsub.listen("A.Recipe.AddPhoto.ClickEvent","PhotoUpload.Controller",m)};var m=function(s){k(s.files)};
var k=function(s){var t=s[0];i();if(t.size>20971520){d.$emit("notify","Photo must be less than 20MB in size.",null,"failure");
return}f.readAsDataUrl(s[0],d).then(function(v){d.userSelectedPhoto=v});var u={template:"/templates/PhotoUploadModal.html",className:"ngdialog-theme-default",scope:d,};
u.scope.description=null;if(d.recipeId==0){d.showMakeProfile=true}else{d.showMakeProfile=false}p.openConfirm(u).then(function(v){g.uploadPhoto({userId:d.userId,recipeId:d.recipeId,description:v.description,file:t}).then(n,l)
})};d.onFileSelect=k;var n=function(s){d.hasUploadedPhoto=true;d.photoUrl=s.smallPhotoUrl;d.largePhotoUrl=s.largePhotoUrl;d.$emit("notify","Photo added. Thank you!",null,"success",null,null,function(){if(b){if(d.isProfilePhoto){e.location.href="/account/updateprofilephoto/?photoid="+s.photoId
}else{if(d.recipeId==0){console.debug("Photo finish, changing location: /photos/"+s.photoId+"/");e.location.href="/photos/"+s.photoId+"/"
}}}});c.$broadcast("onPhotoUpload",d.largePhotoUrl);j.push("add a photo",d.recipeId,"Action Complete",d.uploadFrom)};var l=function(s){d.$emit("notify",s,null,"failure")
};var o=function(s){if(!d.rateButtonIsHighlighted){d.rateButtonIsHighlighted=s}else{if(!d.hasReviewedRecipe){d.rateButtonIsHighlighted=s
}}};c.$on("onRatingClick",function(s,t){o(t)});c.$on("onAddPhoto",function(s,t){d.onFileSelect(t)});c.$on("onReviewSaved",function(s,t){d.hasReviewedRecipe=t
});d.photoUploadToggleRating=h.ensureUserIsLoggedIn(function(){if(d.hasReviewedRecipe){q(true)}else{q(!d.rateButtonIsHighlighted)
}});var q=function(s){c.$emit("onRatingClick",s)};d.$on("onMadeItModalPhotoUpload",function(s,t){d.photoUrl=t;d.hasUploadedPhoto=true
});var i=function(){a.find("[ng-file-select]").val("")}}]);
//
...
var d=new Image();d.onload=function(){c.resolve({width:d.width,height:d.height})};d.src=b;return c.promise}}]).service("ar_services_photoupload",["$upload","datalayerEvent","ar_services_environment","ar_services_token","ar_services_file_reader","ar_services_imagesizer","$q",function(g,f,b,e,c,d,a){var h=960,i=960,k="Photo must be at least 960 x 960.",j="Photo must be less than 20MB in size.";
this.readPhotoData=function(m,n){var l=a.defer();if(m.size>20971520){l.reject({error:j})}else{c.readAsDataUrl(m,n).then(function(o){d.getImageSize(o).then(function(p){if(p.width<i||p.height<h){l.reject({error:k})
}else{l.resolve({dataUrl:o,dimensions:p})}})})}return l.promise};this.calculateOffsets=function(l,o){var n={xOffset:0,yOffset:0};
var p=l.width;var m=l.height;if(p>m){n.xOffset=-(o/m)*(p-m)/2}else{if(m>p){n.yOffset=-(o/p)*(m-p)/2}}return n};this.uploadPhoto=function(m){var l=a.defer();
if(!m.description){m.description=""}g.upload({url:b.url+"v1/recipes/"+m.recipeId+"/photos",headers:{Authorization:function(){var n=e.token();
return n},"Content-Type":undefined},data:{userid:m.userId,description:m.description},file:m.file,}).then(function(n){l.resolve({photoId:n.data.photoId,largePhotoUrl:b.imageServerUrl+"userphotos/250x250/"+n.data.photoId+".jpg",smallPhotoUrl:b.imageServerUrl+"userphotos/70x70/"+n.data.photoId+".jpg"})
},function(o){var p=o.status;var n="We didn't get your photo. Try submitting it again.";if(p!==401){if(o!=""&&o.data.message!="An error has occurred."){n=o.data.message;
f.pushError("add a photo",m.recipeId,"Error Message",o.data.exceptionMessage)}}l.reject(n)});return l.promise}}]);
//
...
//
"use strict";angular.module("allrecipes").directive("sideloadedAd",["ads","$timeout",function(b,a){return{scope:{adSlotId:"=",adSlotType:"=",adSlotHcType:"@",desktopOnly:"="},restrict:"E",replace:true,templateUrl:"/templates/ResponsiveGridAd.html",link:function(d){var c=d.adSlotType||d.adSlotHcType;
a(function(){b.refreshResponsiveAdSlot(d.adSlotId,c,d.desktopOnly)})}}}]);
//
// source/angular/services/feedItemConverter.js
//
angular.module("allrecipes").factory("feedItemConverter",function(){var a=function(g){var f={FacebookRecommendation:"icon--fb-bug",CookRecommendation:"icon--ar-bug"};
var e={FacebookRecommendation:"Find more Facebook friends",CookRecommendation:"You and this cook have similar favorites",RecipeRecommendationSaved:"You've saved similar recipes",RecipeRecommendationViewed:"You've viewed similar recipes",Staffpick:"Allrecipes Staff Pick",RecipeOfTheDay:"Recipe of the Day",RecipeRecommendationPopular:"One of our most popular recipes",SavedRecipeActivity:g.usernameForActivity+" saved this recipe",MadeRecipeActivity:g.usernameForActivity+" made this recipe!",};
var d={FacebookRecommendation:"/cook/my/friends",CookRecommendation:"/cook/my/favorites",RecipeRecommendationSaved:"javascript:void(0);",Staffpick:"javascript:void(0);",RecipeOfTheDay:"javascript:void(0);",RecipeRecommendationViewed:"javascript:void(0);",RecipeRecommendationPopular:"javascript:void(0);",SavedRecipeActivity:"javascript:void(0);",MadeRecipeActivity:"javascript:void(0);",};
var c={FacebookRecommendation:"cook-tile",CookRecommendation:"cook-tile"};var h={FacebookRecommendation:"HalfCook",CookRecommendation:"HalfCook",RecipeOfTheDay:"recipe",Staffpick:"recipe",RecipeRecommendationSaved:"recipe",RecipeRecommendationViewed:"recipe",RecipeRecommendationPopular:"recipe",SavedRecipeActivity:"recipe",MadeRecipeActivity:"recipe",};
var b={FacebookRecommendation:"fb cook",CookRecommendation:"cook recommended",RecipeOfTheDay:"rotd",Staffpick:"staff pick",RecipeRecommendationSaved:"previously saved",RecipeRecommendationViewed:"previously viewed",RecipeRecommendationPopular:"popular",SavedRecipeActivity:"followed cook saved",MadeRecipeActivity:"followed cook made"};
return{iconClass:f[g.feedItemType],headerText:e[g.feedItemType],headerLink:d[g.feedItemType],analyticsType:b[g.feedItemType],submitter:{userID:g.cookId,name:g.cookDisplayName,followersCount:g.followersCount,favoritesCount:g.favoriteCount,madeRecipesCount:g.madeRecipesCount,photo:{urls:[{url:g.thumbnailUrl}]}},imageUrl:g.imageUrl,userID:g.userId||g.id,followersCount:g.followersCount,name:g.cookDisplayName,itemType:h[g.feedItemType],description:g.description,title:g.recipeTitle,recipeID:g.id,id:g.id,photo:{urls:[null,null,null,{url:g.imageUrl}]},ratingAverage:g.rating,reviewCount:g.reviewCount,videoID:g.videoId,adClass:c[g.feedItemType],type:h[g.feedItemType],isSponsored:g.isSponsored,sourceID:g.sourceId}
};return function(c){var d=[];for(var b=0;b<c.length;b++){d.push(a(c[b]))}return d}});
//
...
//
"use strict";angular.module("allrecipes").controller("ar_controllers_create_personal_recipe",["$scope",function(a){a.showKADetails=false;
a.toggleKADetails=function(){a.showKADetails=!a.showKADetails}}]);
//
// source/angular/directives/favoritesNotification.js
//
"use strict";angular.module("allrecipes").directive("favoritesNotification",["ar_services_environment","ipCookie","arLogin",function(a,c,b){return{restrict:"E",replace:true,link:function(f){f.hideFavoritesNotification=false;
var e="AR_Display_FavoritesNotification_Cookie";if(c(e)){f.hideFavoritesNotification=true}else{var d=a.baseDomain;c(e,true,{domain:d,path:"/",expires:180,expirationUnit:"days"})
}f.closeNotification=function(){f.hideFavoritesNotification=true}},templateUrl:function(){if(b.isLoggedIn()){return"/templates/loggedUserFavoritesNotification.html"
}return"/templates/favoritesNotification.html"}}}]);
